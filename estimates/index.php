...
$msdb=mssql_select_db("BUCKINGHAM",$msconnect);
	...
	$hist_arr_rating = array();
 	$hist_ms_qry_maxid = "SELECT dbo.Prod_Issuers.IssuerID, 
	...
	$hist_ms_results_maxid = mssql_query($hist_ms_qry_maxid);
	while ($hist_row_maxid = mssql_fetch_array($hist_ms_results_maxid)) {

      $hist_ms_qry_rating = "select dbo.Issuers.CUSIP, dbo.Prod_Issuers.Recommendation, dbo.Prod_Issuers.IssuerID from dbo.Prod_Issuers
														INNER JOIN dbo.Issuers ON dbo.Prod_Issuers.IssuerID = dbo.Issuers.IssuerID		 
														where dbo.Prod_Issuers.IssuerID = '".$hist_row_maxid[0]."' and dbo.Prod_Issuers.ProductID = '".$hist_row_maxid[1]."'";
			//echo $hist_ms_qry_rating;
			//exit;
			$hist_ms_results_rating = mssql_query($hist_ms_qry_rating);
			while ($hist_row_rating = mssql_fetch_array($hist_ms_results_rating)) {
		  	$hist_arr_rating[$hist_row_rating[0]] = $hist_row_rating[1]; 
			} 
	}

foreach($hist_arr_rating as $k=>$v) {
	$result = mysql_query("insert into _jovus_migration_rating_hist values(NULL,'".$k."','".$v."')");	
}*/

$hist_arr_rating = array();
$result = mysql_query("select * from _jovus_migration_rating_hist order by symbol");
while($row = mysql_fetch_array($result)) {
	$hist_arr_rating[$row["symbol"]] = $row["rating"];
}
...
<title>ESTIMATES COLLATION</title>
<link href="../includes/styles.css" rel="stylesheet" type="text/css" />
</head>
<body leftmargin="0" topmargin="0" rightmargin="0" bottommargin="0">
<table width="100%" border="0" cellpadding="20" cellspacing="0" bgcolor="#000099">
  <tr>
    <td><font face="Verdana, Arial, Helvetica, sans-serif" size="+5" color="#CCCCCC">Estimates Collation</font></td>
  </tr>
</table>
<form name="startproc" action="<?=$PHP_SELF?>" method="POST">
<table width="600" border="0" cellspacing="0" cellpadding="10">
  <tr>
    <td width="156" class="links12_quote">Email Address:</td>
    <td width="404"><input type="text" size="60" name="sendemail" value="<?=$sendto?>" <? if ($_POST) {echo 'disabled="disabled"';}?> /></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td><input type="submit" name="process_estimates"    
				<? 
				if ($_POST) {
					echo 'disabled="disabled"'." ";
					echo 'value="Processing Started ... Please don\'t close Browser Window."/>&nbsp;&nbsp;<a class="links12_quote" href="'.$PHP_SELF.'">[Restart]</a>';
				} else {
					echo 'value="Start Processing" />';
				}
				?>
			</td>
  </tr>
  <tr>
    <td colspan="2">
			<p class="links12_quote" align="justify">
			This utility creates a spreadsheet with estimates from Buckingham and corresponding
			consensus values.<br /><br />
			It may take anywhere from 30 seconds to 10 minutes to complete processing. After completion, 
			this utility will email (to the address provided above) the spreadsheet, as an
			attachment.
			</p>
		</td>
  </tr>
...
if ($_POST) {
	...
	echo "<br>&nbsp;&nbsp;&nbsp;Processing started at ".date('m/d/y h:i:sa')."<br>";
	ob_flush();
	flush();
	...
	$fp = fopen($exportlocation."out.xls", "w");
	...
	$str = '<table width="800" border="1" cellspacing="0" cellpadding="0">
						<tr>
							<td width="80">&nbsp;</td>
							<td width="100"><b>BRG</b></td>
							<td width="100"><b>BRG</b></td>
							<td width="110"><b>BRG</b></td>
							<td width="100"><b>BRG</b></td>
							<td width="100"><b>CONSENSUS</b></td>
							<td width="100"><b>CONSENSUS</b></td>
							<td width="100"><b>CONSENSUS</b></td>
							<td width="100"><b>CONSENSUS</b></td>
							<td width="100">&nbsp;</td>
...
*/
	//Get tickers from Jovus
			...
			if ($v_count_rres == 10000) {
				exit;
						...
						if ($val_symbol != 'xxx') { // and $val_symbol == 'TIN'
						//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
									 ...
									 //exit;
										...
										?>
														...
														//show_array($arr_vals_year_qtr);
														//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
																				//))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
																				//get Max ProductID
														$ms_qry_pid   = "SELECT max(dbo.Prod_FinancialValuesV2.ProductID) from dbo.Prod_FinancialValuesV2
																				 INNER JOIN dbo.Prod_Statuses on dbo.Prod_FinancialValuesV2.ProductID = dbo.Prod_Statuses.ProductID
																				 WHERE dbo.Prod_FinancialValuesV2.IssuerID = '".$row_rres[0]."'
																				 AND dbo.Prod_Statuses.StatusTypeID = 3";
																				...
																				while ($row_pid = mssql_fetch_array($ms_results_pid)) {
																						...
																						$next_year = 'NO EST';
																								...
																								if ($arr_vals_year_qtr[1] < 4) {
																												$ms_qry_relevant_vals = 	"SELECT Q".$arr_vals_year_qtr[1]."Value, Q".($arr_vals_year_qtr[1]+1)."Value, FYValue
																																										FROM dbo.Prod_FinancialValuesV2
																																										WHERE IssuerID = '".$row_rres[0]."'
																																										AND ProductID = '".$row_pid[0]."'
																																										AND ParameterName = 'EPS'
																												...
																												$ms_qry_relevant_vals_more = 	"SELECT FYValue
																																												FROM dbo.Prod_FinancialValuesV2
																																												WHERE IssuerID = '".$row_rres[0]."'
																																												AND ProductID = '".$row_pid[0]."'
																																												AND ParameterName = 'EPS'
																												...
																												$ms_qry_relevant_vals = 	"SELECT Q".$arr_vals_year_qtr[1]."Value, FYValue
																																										FROM dbo.Prod_FinancialValuesV2
																																										WHERE IssuerID = '".$row_rres[0]."'
																																										AND ProductID = '".$row_pid[0]."'
																																										AND ParameterName = 'EPS'
																												...
																												$ms_qry_relevant_vals_more = 	"SELECT Q1Value, FYValue
																																												FROM dbo.Prod_FinancialValuesV2
																																												WHERE IssuerID = '".$row_rres[0]."'
																																												AND ProductID = '".$row_pid[0]."'
																																												AND ParameterName = 'EPS'
													  ...
													  $val_curr_qtr = "";
														...
														if ($arr_rating[$val_IssuerID] == "") {
																$str_show_rating = $hist_arr_rating[$val_symbol];
														} else {
																$str_show_rating = $arr_rating[$val_IssuerID];
														}
														...
														$str = '<tr>'.
																			...
																			'<td>'.$val_curr_year.'</td>'.
																			'<td>'.$val_next_year.'</td>'.
																			'<td>'.$all_table_data[1][1].'</td>'.
																			'<td>'.$all_table_data[1][2].'</td>'.
																			'<td>'.$all_table_data[1][3].'</td>'.
																			'<td>'.$all_table_data[1][4].'</td>'.
																			'<td>'.$str_show_rating.'</td>'.
																			'<td>['.trim(str_replace('Current Qtr','',$all_table_data[0][1])).']</td>'.
																			'<td>'.$var_FYE.'</td>'.
																			'<td>'.$val_dirty.'</td>'.
																		'</tr>';
														fputs ($fp, $str);
														...
														//$str_to_put = '"'..'","'..'","'..'","'..'","'..'","'..'","'..'","'..'","'..'","'..'","['..']","'..'","'..'"'."\n";
														//$str_to_put = '"'.$val_symbol.'","'.$val_curr_qtr.'","'.$val_next_qtr.'","'.$val_curr_year.'","'.$val_next_year.'","'.$all_table_data[1][1].'","'.$all_table_data[1][2].'","'.$all_table_data[1][3].'","'.$all_table_data[1][4].'","'.$str_show_rating.'","['.trim(str_replace('Current Qtr','',$all_table_data[0][1])).']","'.$var_FYE.'","'.$val_dirty.'"'."\n";
														//fputs ($fp, $str_to_put);
										}
										ob_flush();
										flush();
  ...
  fclose($fp);
				...
				$link = "";
				$link = $_site_url."estimates/out.xls";

				?>
        <br>&nbsp;&nbsp;&nbsp;Email Sent with file attached. You can also access the file <a href="<?=$link?>" target="_blank">>>HERE<<</a><br>
        <?
...
*/
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				...
				$arr_attachfile = array("out.xls"=>"D:/tdw/tdw/estimates/out.xls");
				
				zSysMailer("bheller@BuckResearch.com", "Brandon Heller", $subject, $html_body, $text_body, $arr_attachfile) ;
				zSysMailer("npakler@buckresearch.com", "Nik Pakler", $subject, $html_body, $text_body, $arr_attachfile) ;
				zSysMailer("mperillo@buckresearch.com", "Mike Perillo", $subject, $html_body, $text_body, $arr_attachfile) ;
				zSysMailer("pprasad@centersys.com", "Pravin Prasad", $subject, $html_body, $text_body, $arr_attachfile) ;
					zSysMailer($sendemail, "", $subject, $html_body, $text_body, $arr_attachfile) ;
				} else {
					echo "<br><br>No email sent! Email address not provided.";
				}
