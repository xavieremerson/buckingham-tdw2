...
function monthly_report() {
	...
	var url = 'http://192.168.20.63/tdw/bcm_trend_v5_privol_ajx.php';
...
function monthly_report_holding() {

	var sel_month_holding = $("sel_month_holding").value;
	
	if (sel_month_holding == "") {
	  alert("Please select Month/Year.");
		return false;
	}
	...
	var progressbar;
	progressbar = 'Processing. Please do not close this window.<br><br><img src="images/loading-bar.gif" border="0">';
	document.getElementById('notify_holding').innerHTML=progressbar; 
	

	var url = 'http://192.168.20.63/tdw/bcm_trend_v5_privol_ajx_holding.php';
	var pars = 'user_id=<?=$user_id?>';
  pars = pars + '&sel_month='+ sel_month_holding;
  var ran_number= Math.random()*5; 
	pars = pars + '&xrand=' + ran_number;

	//$("test_results").innerHTML = url+"?"+pars;
	//return false;
      ...
      {     
        ...
        onSuccess: 
          function(transport){       
            ...
            response = transport.responseText;       
						$("test_results").innerHTML = response;
						document.getElementById('notify_holding').innerHTML=""; 
          },     
        onFailure: 
        	function(){ 
						$("notify_holding").innerHTML = "Communication Error! Please report with Code TDW-BCM3";
					}
      }
    );
}
...
-->        <tr>
    ...
    <tr>
        ...
        <tr> 
          ...
          calto.addDisabledDates("<?=format_date_ymd_to_mdy(business_day_forward(strtotime(previous_business_day()),1))?>",null);
				  ...
				  var calmonholding = new CalendarPopup("divmonholding");
					calmonholding.setDisplayType("month");
					calmonholding.setReturnMonthFunction("monthReturnHolding");
					calmonholding.showYearNavigation();
					function monthReturnHolding(y,m) {
						document.forms['frm_pvol_holding'].sel_month_holding.value=m+"-"+y;          						
          }
					</SCRIPT>
        <tr> 
    ...
    <tr>
    	<td width="10">&nbsp;</td>
    	<td><hr size="2" noshade color="#006699" /><a class="ilt">Monthly Price/Volume Report<br />With Holdings T-0 thru. T-4</a><br />
      
        <form action="#" method="post" enctype="multipart/form-data" name="frm_pvol_holding" id="frm_pvol_holding">
        <table width="100%">
        <tr> 
          <td><a class="ilt">Select Month</a></td>
          <td><input type="text" class="val_input" id="sel_month_holding" name="sel_month_holding" size="12" maxlength="12" value="<?=date('n-Y')?>">
          <A HREF="#" onClick="calmonholding.showCalendar('anchor4'); return false;" NAME="anchor4" ID="anchor4"><img src="images/lf_v1/sel_date.png" border="0"></A></td>			
          <td>&nbsp;</td>			
        </tr>
        <tr>
        <td align="left" class="csys_regtext">&nbsp;</td>
        <td align="left"><input class="Submit" style="width:150px" name="Save" type="button" value=" SUBMIT "  onclick="monthly_report_holding(); return false;"></td>
        <td>&nbsp;</td>
        </tr>
        <tr><td colspan="3"><div id="notify_holding" class="ilt"></div></td></tr>
        </table>
        </form>
      </td>
    </tr>
...
?>
	...
	<DIV ID="divmonholding" STYLE="position:absolute;visibility:hidden;background-color:white;layer-background-color:white;"></DIV>			
<!-- =================================================================================================================== -->
<!-- END TABLE 2 -->		
</td>
</tr>
...
<script language="javascript">
	function run_test() {
		...
		var url = 'http://192.168.20.63/tdw/bcm_trend_v5_privol_apply_ajx.php';
		var pars = 'mod_request=runtest';
		pars = pars + "&" + $("frm_test").serialize();
		pars = pars + '&xrand='+ Math.random();
		//alert(pars);
		//return false;
		//$("err_data").innerHTML = "bcm_trend_v5_privol_apply_ajx.php?"+pars;
		//return false;  
