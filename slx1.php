...
<?
	...
	while ( $rowx = mysql_fetch_array($result_advisor) ) 
	{
		...
		$result_acct = mysql_query("SELECT distinct(nadd_rr_owning_rep) from nfs_nadd where nadd_short_name = '".$rowx["nadd_short_name"]."' order by nadd_full_account_number, nadd_address_line_1") or die (mysql_error());
		$str_reps = '';
		while ( $rowy = mysql_fetch_array($result_acct) )
		{
				$str_reps .= $rowy["nadd_rr_owning_rep"].";";
		}
		echo ",".$str_reps."\n<br>";
	}
	?>