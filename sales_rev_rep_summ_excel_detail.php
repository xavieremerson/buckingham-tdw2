...
if ($_GET) {
  ...
  $trade_date_to_process = previous_business_day();
	...
	$rep_initials = db_single_val("select Initials as single_val from users where ID = '".$sel_rep."'");
...
//Client current tier and how many years in that tier and if it went up or down.
	...
	while ( $row = mysql_fetch_array($result_get_reps) ) {
	  $arr_reps_initials[$row["Initials"]] = $row["rep_name"];
  ...
  foreach ($arr_active_clients as $k=>$v) {
	...
	//Function to return formatted rep name for clients.
	function get_rep_for_clnt ($arr_reps_initials, $arr_clnt_rep_revenue_tier) {
		if ($arr_clnt_rep_revenue_tier[3] != "") {
			return $arr_reps_initials[$arr_clnt_rep_revenue_tier[2]]." / " . $arr_reps_initials[$arr_clnt_rep_revenue_tier[3]];
		} else {
			return $arr_reps_initials[$arr_clnt_rep_revenue_tier[2]];
		}
	}
...
$output_filename = "sales_rev_summary_detail.xls";
...
$str .= '<table cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" border="1">
     <tr> 
      <td width="250">Rep. Name(s)</td>
      <td width="308">Client Name</td>
      <td width="80">Tier</td>
      <td width="70" align="right">Current Year MTD</td>
      <td width="70" align="right">Previous Year MTD</td>
      <td width="80" align="right">MTD Change</td>
      <td width="70" align="right">Current Year YTD</td>
      <td width="70" align="right">Previous Year YTD</td>
      <td width="70" align="right">YTD Change $</td>
      <td width="70" align="right">YTD Change %</td>
      <td width="70" align="right">Current YTD Annualized</td>
      <td width="70" align="center">Target Budget</td>
      <td width="70" align="center">Perf. vs. Budget</td>
      <td width="80" align="right">Previous Year Full</td>
      <td width="80" align="center">Perf. vs. Previous Year Full</td>
      <td width="80" align="right">Year before Previous Year</td>
    </tr>';
	...
	//exit;
			...
			foreach($arr_active_clients as $cindex=>$ccode) {
					...
					//}
							...
							if ($rep_initials == "") {
											if (get_rep_for_clnt($arr_reps_initials, $clnt_rep_revenue_tier[$ccode])!="") {													
											$str .= '<tr> 
																<td>'.get_rep_for_clnt($arr_reps_initials, $clnt_rep_revenue_tier[$ccode]).'</td>
																<td>'.$clnt_names[$ccode].'</td>
																<td>'.$clnt_rep_revenue_tier[$ccode][1].'</td>
									...
									if ($clnt_rep_revenue_tier[$ccode][2] == $rep_initials || $clnt_rep_revenue_tier[$ccode][3] == $rep_initials) {				
											if (get_rep_for_clnt($arr_reps_initials, $clnt_rep_revenue_tier[$ccode])!="") {													
											$str .= '<tr> 
																<td>'.get_rep_for_clnt($arr_reps_initials, $clnt_rep_revenue_tier[$ccode]).'</td>
																<td>'.$clnt_names[$ccode].'</td>
