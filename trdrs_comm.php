<?
include('trdrs_comm_inc_main.php');
...
<table width="100%" cellpadding="1", cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr>
          <td valign="top"> 
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
              ...
              <tr bgcolor="#333333"> 
                <td width="300"><a class="tblhead_a">&nbsp;&nbsp;&nbsp;&nbsp;ADVISOR / CLIENT (PRIMARY)</a></td>
              ...
              <?
              //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
							//show_array($arr_clients_show);
							
							$arr_clients_show_final_sort = array();
							foreach($arr_clients_show as $k=>$v) {
								$arr_clients_show_final_sort[$v] = look_up_client($v);
							}
							
							asort($arr_clients_show_final_sort);
							//show_array($arr_clients_show_final_sort);
							
							
							
							$level_a_count = 0;
							//foreach($arr_clients_show as $zk=>$zv) {
							foreach($arr_clients_show_final_sort as $zv=>$zk) {	
								if ($zv != '' 
										AND ( (int)get_previous_yr_data($zv) > 0 OR ( $arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv] ) > 0 )
										) {  //AND get_previous_yr_data($tmp_arr_vals[0]) > 0 AND ( $arr_ytd_comm[$k]+$arr_ytd_check[$k] ) > 0

										if (
													( (int)date('m') > 6 AND ($arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv]) > 0 )
														OR
													( (int)date('m') <= 6 AND ($arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv]) > 0)  
										){
										...
										if(get_previous_yr_data($zv)> 0) {
											$pyc_percent = number_format((($arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv])/get_previous_yr_data($zv))*100,0,'','');
										} else {
											$pyc_percent = 0;
										}
								...
								?>
									<tr class="<?=$class_row?>" >
                    <td valign="middle">&nbsp;&nbsp;<a href='#'><img src="images/t12m_s.png" border="0" onclick="CreateWnd('chart_t12m.php?clnt=<?=$zv?>', 620, 330, false);"></a>
                    &nbsp;
                    <?=substr(look_up_client($zv),0,30)?></td>
                    <td>&nbsp;&nbsp;<?=$final_display_client[$zv]?></td>
                    <td align="right"><?=show_numbers($arr_day_comm[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_mtd_comm[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_qtd_comm[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_ytd_comm[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_mtd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_qtd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_ytd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_mtd_comm[$zv]+$arr_mtd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_qtd_comm[$zv]+$arr_qtd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers($arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv])?>&nbsp;&nbsp;</td>
                    <td align="right"><?=show_numbers(get_previous_yr_data($zv))?>&nbsp;&nbsp;</td>
                    <td align="right"><?=$pyc_percent?>&nbsp;&nbsp;&nbsp;&nbsp;</td> 
                    <td align="right">&nbsp;</td>
                  </tr>
                  <?
                  $total_pbd = $total_pbd + $arr_day_comm[$zv];
                  $total_mtd = $total_mtd + $arr_mtd_comm[$zv];
                  $total_qtd = $total_qtd + $arr_qtd_comm[$zv];
                  $total_ytd = $total_ytd + $arr_ytd_comm[$zv];
                  $total_cmtd = $total_cmtd + $arr_mtd_check_new[$zv]; 
                  $total_cqtd = $total_cqtd + $arr_qtd_check_new[$zv]; 
                  $total_cytd = $total_cytd + $arr_ytd_check_new[$zv];  
                  $total_grand_mtd = $total_grand_mtd + $arr_mtd_comm[$zv]+$arr_mtd_check_new[$zv];
                  $total_grand_qtd = $total_grand_qtd + $arr_qtd_comm[$zv]+$arr_qtd_check_new[$zv];
                  $total_grand_ytd = $total_grand_ytd + $arr_ytd_comm[$zv]+$arr_ytd_check_new[$zv];
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
               <tr class="display_totals"> 
                <td width="300" align="left">&nbsp;&nbsp;TOTALS:</td>
                <td width="40">&nbsp;&nbsp;</td>
  ...
  <tr id="pbd">
    <td>
      <br>
      <?
      //include('trdrs_comm_inc_trade.php');
