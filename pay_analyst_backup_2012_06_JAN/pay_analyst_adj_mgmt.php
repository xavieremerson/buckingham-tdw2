...
<script language="javascript">
	function submit_adj() {
		...
		var cname_psv;
		var str_message = "You submission is incorrect or incomplete, hence cannot be saved.\n";
		str_message = str_message + "\nEach column of adjustment MUST NET TO ZERO.\n";
		str_message = str_message + "\nThe following Clients have incomplete or incorrect values\n";
		...
		for (i=1;i<=ccount;i++) {
			totalid = "total|"+ i;			
			//alert (totalid);
			if (document.getElementById(totalid).innerHTML != "0.00%") {
		...
		if (j == 0) {
			if(confirm('Do you want to proceed with saving the adjustments?')) {
...
<?
include('pay_analyst_adj_js.php');
include('pay_analyst_adj_css.php');
...
if ($sel_rep) {
	...
	$str_heading = "Analyst Allocations Adjustments : ". $str_name;
} else {
	$str_heading = "Analyst Allocations Adjustments";
}
...
<table width="100%" border="0" cellpadding="4" cellspacing="0"> 
	<tr>
		...
		?>
							<option value="<?=$row_get_reps["rr_num"]."^".$row_get_reps["ID"]?>"  
							               <? if ($sel_rep == $row_get_reps["rr_num"]."^".$row_get_reps["ID"]) { echo "selected";}?>>
												     <?=str_pad($row_get_reps["rep_name"], 20, ".")?>(<?=$row_get_reps["rr_num"]?>)</option>
		<?
		}
		?>
		</select>
      ...
      <?
			for ($i=-2;$i<2;$i++) {
			?>
			<option value="<?=date('Y')+$i?>" <? if ($sel_year == date('Y')+$i) { echo "selected";}?>><?=date('Y')+$i?></option>
			<?			
			}
			?>
		</select>		
...
include('pay_analyst_adj_inc_main.php');

include('pay_analyst_adj_inc_main_more.php');
...
//=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>=<>
				...
				for ($j=1;$j<=$frmitem_ccount;$j++) {
					$str_varname = $i."|".$j;
						...
						if ($val_count == 0) {	//insert													
								$qry_insert = "INSERT INTO pay_analyst_allocations_adj ( 
															auto_id, 
															pay_qtr,
															pay_year,
															pay_final,
								...
								$qry_update = "UPDATE pay_analyst_allocations_adj
															SET
				...
				for ($j=1;$j<=$frmitem_ccount;$j++) {
					$str_varname = $i."|".$j;
						...
						$val_count = db_single_val("select count(*) as single_val 
																				...
																				and pay_advisor_code = '".$arr_val_clients[$j]."'
																				and pay_analyst_id = '".$arr_val_analysts[$i]."'");
						if ($val_count == 0 and $$str_varname != 0) {	//insert													
								$qry_insert = "INSERT INTO pay_analyst_allocations_adj ( 
															auto_id, 
															pay_qtr,
															pay_year,
								...
								$qry_update = "UPDATE pay_analyst_allocations_adj
															...
															and pay_analyst_id = '".$arr_val_analysts[$i]."'";
								$result_update = mysql_query($qry_update) or die (tdw_mysql_error($qry_update));
								} else { //remove the record from the table
								$qry_update = "UPDATE pay_analyst_allocations_adj
															SET
															pay_isactive = 0
															WHERE pay_qtr = '".$frmitem_qtr."'
	...
	if ($frm_saved == 1 OR $frm_finalized == 1 or 1==1) { // get saved or finalized data 
		...
		//ORIGINAL DATA
		$arr_original = array();
		$qry_get_original = "select * from pay_analyst_allocations
											where pay_qtr = '".$sel_qtr."'
											and pay_year = '".$sel_year."'
											and pay_sales_id = '".$user_sales."'";
		//echo $qry_get_data;
		$result_get_original = mysql_query($qry_get_original) or die (tdw_mysql_error($qry_get_original));
		while ( $row_get_original = mysql_fetch_array($result_get_original) ) 
		{
			$arr_original[$row_get_original["pay_analyst_id"]][$row_get_original["pay_advisor_code"]] = round($row_get_original["pay_percent"],2);
		}
		//==============================================================================
  ...
  //ORIGINAL
	//populate array in the form of $i|$j to populate values.
	if ($frm_saved == 1 OR $frm_finalized == 1 or 1==1) {
		$newform = 0;
		$rcount = 1;
		$pop_array_original = array();
		foreach ($arr_analysts as $ka=>$va) {
			$ccount = 1;
			foreach ($arr_master_clnt_rr as $kc=>$vc) {
			if ($arr_original[$ka][$kc] == "") { //This accommodates situations where BRG Non allocable data is not present.
				$pop_array_original[$rcount."|".$ccount] = 0;
			} else {
				$pop_array_original[$rcount."|".$ccount] = $arr_original[$ka][$kc];
			}
			$ccount++;
			}
		$rcount++;
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	if ($frm_saved == 1 or $newform == 1 or 1==1) { //form has been initiated or saved but NOT finalized
	?>
			...
			<tr>
				<td width="20" valign="top"> 
					...
					<table class="tbl_xl" border="0" cellpadding="0" cellspacing="0" >
						...
						<tr>
							<td width="1"><img src="images/spacer.gif" width="1" height="25" /></td>
							<td class="tdx" nowrap="nowrap">Net Adj. (%)</td>
						</tr>		
					</table>
					<!-- End Analyst List -->
				</td>
				...
				<div id="scrollGrid_1">
					<table class="tbl_xl" border="0" cellpadding="0" cellspacing="0">
						...
						$str_debug = "";
						$j = 1;
						$hold_total_commission_original = array();
						foreach ($arr_master_clnt_rr as $k=>$v) {
						...
						<?
						$hold_total_commission_original[$j] = $total_commission;
						$j = $j + 1;
						}
						?>
						</tr>
							...
							<td class="tdx">
								<table width="100%">
									<tr>
										...
										<td class="num_1" nowrap="nowrap"><?=$pop_array_original[$i."|".$j]?>% [$<?=number_format(($hold_total_commission_original[$j]*$pop_array_original[$i."|".$j])/100,2,".",",")?>]</td>
									</tr>
								</table>
							</td>		
							<?
				...
				<!-- SUMMARY SECTION -->
					<table class="tbl_xl" border="0" cellpadding="0" cellspacing="0" >
						...
						<tr>
							...
							<td class="tdx" nowrap="nowrap" align="right">&nbsp;&nbsp;<a class="pplname">$ Adjusted</a>&nbsp;&nbsp;</td>
...
-->						<img src="images/spacer.gif" width="10" height="1" />
						<input type="image" src="images/btn_save_adj.png" onclick="return submit_adj()"/> 
				</td>
			</tr>
		</table>
