...
include('pay_xnsumm_sdate_functions.php');
...
include('pay_xnsumm_sdate_error_checks.php');
//+ + & & + + & & + + & & + + & & + + & & + + & & + + & & + + & & + + & & + + & & + +
...
records to a single record with the sum of dollars and the single record will be the one with the max of the dates.
*/
...
// 
//OUTSTANDING TODO THE  DATES BELOW SHOULD BE CALENDAR BASED NOT BROKERAGE BASED, FOR NOW, LETS SEE
...
//echo "master_arr_commissions"."<br />";				
...
//echo "master_arr_commission_v1"."<br />";
//show_array($master_arr_commission_v1);
//exit;
//==============================================================================================================================================
...
//echo "master_arr_commission_v2"."<br />";
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// This has been updated because 300 and up are also used for shared reps.
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
foreach ($master_arr_reps as $k=>$v) {
	...
	} elseif (substr($v,0,1)=='4') { //error condition 400 cannot exist

			//$master_arr_users["ERR"] = "ERR"; 
			echo "ERROR CONDITION IN REP LIST : CHECK " . $v . "<BR>"; 
			  
	} elseif (substr($v,0,1)=='5') { //500 series rep numbers not decided yet
...
//echo "master_arr_users_id_names"."<br />";
...
//echo "master_arr_commission_v3"."<br />";
//show_array($master_arr_commission_v3);
//exit;
...
foreach($arr_rep_shared_number as $k=>$v) {
  if(!in_array($k,$prefinal_rep_list)) {
		$str_val_concat = $k."^".get_userid_for_rr ($k)."^".""."^".get_name_for_user_id(get_userid_for_rr ($k))."^".""."^".""."^".""."^".""."^".""."^".""."^".""."^".$v;
	  $master_arr_commission_v4[$k] = $str_val_concat;
	}
}

//echo "master_arr_commission_v4"."<br />";
...
$wkb = new Spreadsheet_Excel_Writer('data/xls/'.$xlfilename);
	...
	include('pay_xnsumm_sdate_gen_excel_format.php');
	...
	include('pay_xnsumm_sdate_excel_header.php');

			$arr_master_new = $master_arr_commission_v4;		
			//include('pay_summ_gen_excel_inc_1.php');
			include('pay_xnsumm_sdate_gen_excel_inc_1.php');
?>