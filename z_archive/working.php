...
$file_output = date('mdy')."_output.pri";
	...
	$fp = fopen ("//bucknypdc3/d$/Port/BuckCap/Pricing/Axys/".$file_output, "w");  
	...
	for ($j=0;$j<$count_total;$j++) {
			...
			*/
			...
			$str_val =  $arr_company[0] . "\t" . $arr_company[1] . "\t" .$$var_price. "\t" . $$var_desc . chr(13);
			fwrite ($fp,$str_val);        
			
			} else {
			
			$str_val =  $arr_company[0] . "\t" . $arr_company[1] . "\t" .$$var_price_val. "\t" . $$var_desc . chr(13);
			fwrite ($fp,$str_val);        
  ...
  <br><br><br><br><strong>&nbsp;&nbsp;&nbsp;&nbsp;Output File : <?=$file_output?> created and saved.</strong>
  </font><br>
  <?	
...
<table border="0" cellpadding="10">
	<tr>
  	<td>
			<?
      if (!is_file("//bucknypdc3/d$/Port/BuckCap/Pricing/Axys/".$file_input)) {
				?>
      ...
      <table class="sample">
			...
			$fcontents = file("//bucknypdc3/d$/Port/BuckCap/Pricing/Axys/".$file_input);
