					   ...
					   //do nothing
							...
							while ($row_client_comm = mysql_fetch_array($result_client_comm) ) 
							{
									...
									foreach ($arr_checks as $clnt=>$amt) {
									   if ($clnt == $row_client_comm["trad_advisor_code"]) {
											...
											$wks->writeNumber($count_row_i, 8, $arr_checks[$row_client_comm["trad_advisor_code"]],$format_currency_1);
										 }
									}
									
									//-----------------------------------------------------------------------------
									$wks->writeFormula($count_row_i, 9, '='.$arr_xl_cols[6].($count_row_i+1)."+".$arr_xl_cols[8].($count_row_i+1) ,$format_currency_1);
									$wks->writeFormula($count_row_i, 11, '='.$arr_xl_cols[9].($count_row_i+1)."*".$payout_multiplier,$format_currency_1);
									//-----------------------------------------------------------------------------
									...
									if ($rolling_total > 15000) {
									$cond_format = $format_currency_1;
										$wks->writeFormula($count_row_i, 15, '='.$arr_xl_cols[11].($count_row_i+1)."*1", $cond_format);
										$wks->writeFormula($count_row_i, 20, '='.$arr_xl_cols[11].($count_row_i+1)."*1",$format_currency_1);
									} else {
									$cond_format = $format_currency_1;
										$wks->writeFormula($count_row_i, 15, '='.$arr_xl_cols[11].($count_row_i+1)."*(-1)", $cond_format);
										$wks->writeFormula($count_row_i, 20, '='.$arr_xl_cols[11].($count_row_i+1)."*0",$format_currency_1);
									}
									//-----------------------------------------------------------------------------
							...
							foreach ($arr_checks_rr as $clnt=>$rr) {
								 if (!in_array($clnt,$processed_client) AND $rr == $row_trades["trad_rr"]) {
									...
									$wks->write($count_row_i, 4, $clnt_name,$format_data_1);
									$wks->writeNumber($count_row_i, 8, $arr_checks[$clnt],$format_currency_1);
									$wks->writeFormula($count_row_i, 9, '='.$arr_xl_cols[6].($count_row_i+1)."+".$arr_xl_cols[8].($count_row_i+1) ,$format_currency_1);
									$wks->writeFormula($count_row_i, 11, '='.$arr_xl_cols[9].($count_row_i+1)."*".$payout_multiplier,$format_currency_1);
							...
							$wks->writeFormula($count_row_i, 9,  '=SUM('.$arr_xl_cols[9]."5".":".$arr_xl_cols[9].($count_row_i).")",$format_currency_2);
							$wks->writeFormula($count_row_i, 11, '=SUM('.$arr_xl_cols[11]."5".":".$arr_xl_cols[11].($count_row_i).")",$format_currency_2);
							$wks->writeFormula($count_row_i, 15, '=SUM('.$arr_xl_cols[15]."5".":".$arr_xl_cols[15].($count_row_i).")",$format_currency_2);
							$wks->writeFormula($count_row_i, 20, '=SUM('.$arr_xl_cols[20]."5".":".$arr_xl_cols[20].($count_row_i).")",$format_currency_2);
							$count_row_i = $count_row_i + 1;
						  ...
						  //Now get the shared reps for the primary rep
								...
								//$wks->writeRow($count_row_j,0," ");
												...
												//****
													...
													foreach ($arr_checks as $clnt=>$amt) {
														 if ($clnt == $row_client_comm_s["trad_advisor_code"]) {
															//echo "Client ".$clnt. " found with Check Amount = ". $amt."<br>";
															$wks->writeNumber($count_row_k, 8, $arr_checks[$row_client_comm_s["trad_advisor_code"]],$format_currency_1);
														 }
													}
													
													//-----------------------------------------------------------------------------
													$wks->writeFormula($count_row_k, 9, '='.$arr_xl_cols[6].($count_row_k+1)."+".$arr_xl_cols[8].($count_row_k+1) ,$format_currency_1);
													$wks->writeFormula($count_row_k, 11, '='.$arr_xl_cols[9].($count_row_k+1)."*".$payout_multiplier,$format_currency_1);
													//-----------------------------------------------------------------------------
													//get rolling 12 months data
													...
													if ($rolling_total > 15000) {
													$cond_format = $format_currency_1;
														$wks->writeFormula($count_row_k, 15, '='.$arr_xl_cols[11].($count_row_k+1)."*1", $cond_format);
														$wks->writeFormula($count_row_k, 20, '='.$arr_xl_cols[11].($count_row_k+1)."*1",$format_currency_1);
													} else {
													$cond_format = $format_currency_1;
														$wks->writeFormula($count_row_k, 15, '='.$arr_xl_cols[11].($count_row_k+1)."*(-1)", $cond_format);
														$wks->writeFormula($count_row_k, 20, '='.$arr_xl_cols[11].($count_row_k+1)."*0",$format_currency_1);
													}
									...
									foreach ($arr_checks_rr as $clnt=>$rr) {
										 if (!in_array($clnt,$processed_client_shared) AND $rr == $row_trades_shared["trad_rr"]) {
											...
											$wks->writeNumber($count_row_k, 8, $arr_checks[$clnt],$format_currency_1);
											$wks->writeFormula($count_row_k, 9, '='.$arr_xl_cols[6].($count_row_k+1)."+".$arr_xl_cols[8].($count_row_k+1) ,$format_currency_1);
											$wks->writeFormula($count_row_k, 11, '='.$arr_xl_cols[9].($count_row_k+1)."*".$payout_multiplier,$format_currency_1);
							...
							$wks->writeFormula($count_row_j, 8,  '=SUM('.$arr_xl_cols[8].($hold_count_j+1).":".$arr_xl_cols[8].($count_row_j).")",$format_currency_2);
							$wks->writeFormula($count_row_j, 9,  '=SUM('.$arr_xl_cols[9].($hold_count_j+1).":".$arr_xl_cols[9].($count_row_j).")",$format_currency_2);
							$wks->writeFormula($count_row_j, 11, '=SUM('.$arr_xl_cols[11].($hold_count_j+1).":".$arr_xl_cols[11].($count_row_j).")",$format_currency_2);
							
							$wks->writeFormula($count_row_j, 15, '=SUM('.$arr_xl_cols[15].($hold_count_j+1).":".$arr_xl_cols[15].($count_row_j).")",$format_currency_2);
							$wks->writeFormula($count_row_j, 20, '=SUM('.$arr_xl_cols[20].($hold_count_j+1).":".$arr_xl_cols[20].($count_row_j).")",$format_currency_2);
...
?>
<a href="http://192.168.20.63/tdw/data/xls/<?=$xlfilename?>" target="_blank">Get Excel File</a><br />
<?
