...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//ONE TIME FETCH OF ALL EMAILS IN 3 MAILBOXES TO ARRAYS
    ...
    show_array($arr_iu_date_subject);
		exit;

		
		//3333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333
		//BuckQuicknote
		$obj= new receiveMail('BuckQuicknote@buckresearch.com','286$brMail','BuckQuicknote@buckresearch.com','owa.smarshexchange.com','imap','143',false); 
		//Connect to the Mail Box
		$obj->connect(); 
		// Get Total Number of Unread Email in mail box
		$tot=$obj->getTotalMails(); 
...
/*[3]
0 = [Mark Weintraub]
1 = ["research dissemination(distro list)" ]
2 = [Investment Update: OSB flat to up; lumber up modestly]
3 = [Fri, 18 Jan 2013 14:42:12 -0500]
4 = [01/18/2013 02:42pm]
5 = [2013-01-18]
*/

/*	$arr_investment_updates = array();
	$arr_quick_notes = array();

	$needle_1 = "investment update";
	$needle_2 = "quick note";
	foreach ($arr_rd_emails as $k=>$v) {
		if (stripos($v[2], $needle_1)===false) { 
			//echo "NO MATCH >> INVESTMENT UPDATE >> ". $v[2]."\n";
			if (stripos($v[2], $needle_2)===false) { 
				//echo "NO MATCH >> QUICK NOTE >> ". $v[2]."\n";
			} else {
				//echo "MATCH >> QUICK NOTE >> ". $v[2]."\n"; 
				$arr_quick_notes[] = $v[5]."^".$v[2];
			}		
		} else { 
			//echo "MATCH >> INVESTMENT UPDATE >> ". $v[2]."\n"; 
			$arr_investment_updates[] = $v[5]."^".$v[2];
		}
	}

	show_array($arr_investment_updates);
	show_array($arr_quick_notes);
	exit;*/
...
for ($bizdays=1; $bizdays < 300; $bizdays++) {
	if (strtotime(business_day_backward(strtotime($start_date_seed),$bizdays+1)) < strtotime("2012-04-13")) {
		...
		//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
										...
										<style type="text/css">
										<!--
										.tdwthbrdr {  font-family: Verdana, Arial, Helvetica, sans-serif;  font-size: 11px;  color: #000000;  border-collapse: collapse;border: 1px #777777 solid}
										.fblue {  font-family: Verdana, Arial, Helvetica, sans-serif;  font-size: 11px;  color: #0000ff;}
										-->
										</style>';
										...
										//set all data in the table to inactive for this trade date and then insert this record.
													...
													$result_rem_duplicate = mysql_query("delete from eml_research_compliance where eml_trade_date = '".$trade_date_to_process."'");
