...
$exchange_server = "https://owa.smarshexchange.com";  //evs1
$exchange_username = "tdw@buckresearch.com"; 
$exchange_password = "BRmail678"; 
...
<a:searchrequest xmlns:a=\'DAV:\'>
 <a:sql>
	SELECT "DAV:href","urn:schemas:httpmail:subject","urn:schemas:httpmail:date"
		FROM scope(\'shallow traversal of "'.$exchange_server.'/owa/tdw@buckresearch.com/Inbox"\')
...
<a:searchrequest xmlns:a="DAV:" xmlns:s="http://schemas.microsoft.com/exchange/security/"> 
   <a:sql> 
       ...
       FROM "'.$exchange_server.'/owa/tdw@buckresearch.com/inbox" 
   </a:sql> 
</a:searchrequest>'; 
...
if (!$h->fetch($exchange_server."/owa/tdw@buckresearch.com/inbox", 0, null, $exchange_username, $exchange_password, "SEARCH")) { 
