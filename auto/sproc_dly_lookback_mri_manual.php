...
$trade_date_to_process = '2014-02-12';
ydebug("trade_date_to_process",$trade_date_to_process); //$trade_date_to_process = '2008-07-15';

//CREATE ARRAY FOR LOOKBACK TABLE

$qry_lkbk = "select * from mri_research_ioc where report_date = '".$trade_date_to_process."' and (report_type = 'IOC' or report_type = 'MRI')";
$result_lkbk = mysql_query($qry_lkbk) or die (tdw_mysql_error($qry_lkbk));
$arr_show_lookback_symbols = array();
while ( $row = mysql_fetch_array($result_lkbk) ) { 
	$arr_show_lookback_symbols[] = $row["symbol"];
}

print_r($arr_show_lookback_symbols);

//include('sproc_dly_rep_dcarv2_lookback.php');
?>
				                ...
				                <style type="text/css">
											...
											foreach($arr_lookback as $k=>$lkbk_symbol) {
													...
													if (strlen(trim($lkbk_name)) == 0) {
														//get company name
														$lkbk_name = get_company_name_yhoo($lkbk_symbol);  //FROM YAHOO FINANCE
														if (strlen($lkbk_name) > 50) {
															echo "\n".$lkbk_name."\n";
															if (strlen(trim($lkbk_name)) == 0) {
																$lkbk_name = "";
															} else {
...
//production
	$file_name = "lookback_".$trade_date_to_process.".html";     
	$file_pdf_name = "lookback_".$trade_date_to_process.".pdf";     
...
//$arr_recipient[1] = 'compliance@buckresearch.com';



//$arr_recipient[2] = 'rdaniels@buckresearch.com';
...
foreach ($arr_recipient as $key => $emailval) {
				...
				$subject = "MRI Lookback Report : (Trade Date: ".format_date_ymd_to_mdy($trade_date_to_process).")";
				$text_body = $subject;
