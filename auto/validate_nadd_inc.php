...
$email_log .= '<hr align="left" width="800" size="1" noshade color="#666666">
               <strong>ALERT 1: Advisors with Multiple Reps.</strong>
							 <hr align="left" width="800" size="1" noshade color="#666666">'.
							 "Rep. Numbers with fewer occurences within a client have been displayed in <b>BOLD</b>.<br><br>\n";
				...
				//get count of all RRs for this Advisor
					...
					$numreps = $row_adv_rrcount["numreps"];
						if ($numreps > 2) {
								...
								$email_log .= "<table width='600' class='datatable'>"; 
								$email_log .= "<tr>
																<td><u>Client Code</u></td>
																<td><u>Account Short Name</u></td>
																<td><u>Account Number</u></td>
																<td><u>Rep #</u></td>
															 </tr>\n";
								...
								if ($row_adv["nadd_advisor"] == 'GART') {
										...
										if ($count_rrmin <= $count_rrmax) {
														...
														while($row_display_rrmin = mysql_fetch_array($result_display_rrmin)){
															...
															$email_log .= "<tr".$bkclr.">
																							<td>".$row_display_rrmin["nadd_advisor"]."</td>
																							<td> [".$row_display_rrmin["nadd_short_name"]."]</td>
																							<td>".$row_display_rrmin["nadd_full_account_number"]."</td>
																							<td><b>".$row_display_rrmin["nadd_rr_owning_rep"]."</b></td>
																							</tr>\n";
														...
														while($row_display_rrmax = mysql_fetch_array($result_display_rrmax)){
															...
															$email_log .= "<tr".$bkclr.">
																							<td>".$row_display_rrmax["nadd_advisor"]."</td>
																							<td> [".$row_display_rrmax["nadd_short_name"]."]</td>
																							<td>".$row_display_rrmax["nadd_full_account_number"]."</td>
																							<td>".$row_display_rrmax["nadd_rr_owning_rep"]."</td>
																						 </tr>\n";
														  $count_row_table = $count_row_table + 1;
														}
										} else {
...
while ( $row_get_clients = mysql_fetch_array($result_get_clients) )
				{
					...
					while ( $row_get_rr = mysql_fetch_array($result_get_rr) )
									...
									//echo $row_get_rr["nadd_rr_exec_rep"]."<br>";
											...
											//Now the following information flows from the cron.php file
