...
//show_array($arr_emp_approved_list);
//show_array($arr_emp_approved_time);
//============================================================================================
//============================================================================================
...
while ($row = mysql_fetch_array($result)) {
	...
	$arr_client_symbols_time[$row["Ticker"]] = $row["manual_time"];
}
	...
	if (in_array($v,$arr_client_symbols)) {
	  /*echo $arr_emp_approved_time[$k]."/".
		     strtotime($arr_emp_approved_time[$k]).
				 ...
				 "<br>";*/
		 //xdebug("emp_approved_time",$arr_emp_approved_time[$k])."\n";
		 //xdebug("client symbol time",$arr_client_symbols_time[$v])."\n";
		...
		//xdebug("z_client_time",$z_client_time);
		//echo date('H:i:sa',$z_client_time);
		
		if (strtotime($arr_emp_approved_time[$k]) > strtotime($arr_client_symbols_time[$v])) {
			
				if (round(nicetime($arr_client_symbols_time[$v], $arr_emp_approved_time[$k] ),0) < 40) { //time has to be about half hour or less.
					echo "<br>Emp approved time = ".date('m/d/Y h:i:sa',strtotime($arr_emp_approved_time[$k]));
					//echo "<br>Emp approved time = ".$arr_emp_approved_time[$k]."<br>";
					echo "<br>Client time = ".date('m/d/Y h:i:sa',strtotime($arr_client_symbols_time[$v]))."<br>";
					//echo "<br>Client time = ".$arr_client_symbols_time[$v]."<br>";
					echo "Difference = ".round(nicetime($arr_client_symbols_time[$v], $arr_emp_approved_time[$k] ),0)."<br>";
					//echo "<br>Difference = ".date('m',strtotime($arr_emp_approved_time[$k]) - strtotime($arr_client_symbols_time[$v]))."<br>";
					$arr_matched[] = $k;
				}
  ...
  $arr_tradeware_buy_sell = array('1'=>'Buy',
	...
	foreach($arr_matched as $k=>$v) {
		...
		$qry = "select 
						...
						WHERE auto_id = '".$v."'"; 
		//xdebug("qry",$qry);
		$resultx = mysql_query($qry) or die(tdw_mysql_error($qry));
		while ($row = mysql_fetch_array($resultx)) {
			...
			//xdebug("ZZZZ c_client_rep",$c_client_rep);
			
			$arr_c_reps = explode("^",$c_client_rep);
			//print_r($arr_c_reps);
			
			if (trim($c_client_rep) != "") {
