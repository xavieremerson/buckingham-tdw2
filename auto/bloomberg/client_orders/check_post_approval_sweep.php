...
$qry = "select distinct(Ticker) as Ticker, max(last_trade) as manual_time from client_orders_street group by Ticker"; 
$result = mysql_query($qry) or die(tdw_mysql_error($qry));
while ($row = mysql_fetch_array($result)) {
...
if (count($arr_matched) > 0) {
  ...
  $arr_tradeware_buy_sell = array('1'=>'Buy',
	...
	foreach($arr_matched as $k=>$v) {
		...
		while ($row = mysql_fetch_array($result)) {
			...
			$qry_c = "select
									...
									buy_sell,
									customer_id,
									max(last_trade) as manual_time,
									parent_id
								FROM client_orders_street 
			...
			$email_log .= '<br><a class="bodytext12">'.
												...
												Buy/Sell: <strong>".$row_c["buy_sell"]."</strong><br>
    ...
    zSysMailer("BWilson@buckresearch.com", "Brad Wilson", $subject, $html_body, $text_body, "") ;
