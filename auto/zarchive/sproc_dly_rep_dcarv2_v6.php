...
//====================================================================================================
	...
	//$trade_date_to_process = '2007-03-15';
...
/*
											...
											INNER JOIN dbo.Prod_Statuses ON dbo.Products.ProductID = dbo.Prod_Statuses.ProductID
											INNER JOIN dbo.ExchangeSecurities ON dbo.Issuers.IssuerID = dbo.ExchangeSecurities.SecurityID
											WHERE dbo.Issuers.CUSIP <> '' AND ((dbo.Products.CreationDateTime) <  CAST(FLOOR(CAST(convert(datetime,'".format_date_ymd_to_mdy(date('Y-m-d'))."',120) AS float)) as datetime))
											GROUP BY dbo.Prod_Issuers.IssuerID, dbo.ExchangeSecurities.Ticker, dbo.Prod_Statuses.StatusTypeID
											HAVING (((dbo.Prod_Statuses.StatusTypeID)=3))
											order by dbo.ExchangeSecurities.Ticker;";
											
											xdebug("sql",$ms_qry_rres);
											*/
											//CAST(FLOOR(CAST(convert(datetime,'03-20-2007',120) AS float)) 
											//WHERE dbo.Issuers.CUSIP <> '' AND ((dbo.Products.CreationDateTime) <  CAST(FLOOR(CAST(getdate() AS float)) as datetime))
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  //Most recent MRI Date
		...
		$ms_qry_mri = "SELECT 
										...
										INNER JOIN dbo.Prod_Statuses ON dbo.Products.ProductID = dbo.Prod_Statuses.ProductID
										INNER JOIN dbo.ExchangeSecurities ON dbo.Issuers.IssuerID = dbo.ExchangeSecurities.SecurityID
									  WHERE (((dbo.Issuers.CUSIP)<>'') AND (dbo.Products.CreationDateTime BETWEEN (CAST(FLOOR(CAST(convert(datetime,'".format_date_ymd_to_mdy($trade_date_to_process)."',120) AS float)) as datetime)-180) AND CAST(FLOOR(CAST(convert(datetime,'".format_date_ymd_to_mdy($trade_date_to_process)."',120) AS float)) as datetime)) 
											AND ((dbo.Prod_Statuses.StatusTypeID)=3))
										ORDER BY dbo.ExchangeSecurities.Ticker, dbo.Prod_Statuses.DateTime DESC;";	
		
										xdebug("sql",$ms_qry_mri);

										/*
										WHERE (((dbo.Issuers.CUSIP)<>'') AND (dbo.Products.CreationDateTime BETWEEN (GETDATE()-180) AND CAST(FLOOR(CAST(getdate() AS float)) as datetime)) 
											...
											WHERE (((dbo.Issuers.CUSIP)<>'') AND (dbo.Products.CreationDateTime BETWEEN (GETDATE()-180) AND CAST(FLOOR(CAST(getdate() AS float)) as datetime)) 
												AND ((dbo.Prod_Statuses.StatusTypeID)=3))
											ORDER BY dbo.Issuers.CUSIP, dbo.Prod_Statuses.DateTime DESC;";
		*/
...
//show_array($arr_acct_emp);
	     ...
	     xdebug("total_count_acv_events",$total_count_acv_events);
			 ...
			 $report_header_string = '<font color="#000000" size="3" face="Verdana, Arial, Helvetica, sans-serif"><strong>POTENTIAL ANALYST TRADE VIOLATIONS : 0</strong></font>';
			 } else {
			 $report_header_string = '<font color="#FF0000" size="3" face="Verdana, Arial, Helvetica, sans-serif"><strong>POTENTIAL ANALYST TRADE VIOLATIONS : '.$total_count_acv_events.'</strong></font>';
			 }
...
//				
				...
				//Õ  ß
						...
						if ($rec_to_process[10] == 'YES') {
						$str_bgcolor = " bgcolor='CCCCCC'";
						} else {
...
//test
	...
	$file_name = $trade_date_to_process."_dcarv2_testnew.html";    //  _testnew
	$file_pdf_name = $trade_date_to_process."_dcarv2_testnew.pdf"; //_testnew 
...
$cmd = "copy d:\\tdw\\tdw\\data\\compliance\\".$file_pdf_name. " \\\\bucknypdc3\\compliance$\\TDW_ComplianceReports\\".$file_pdf_name;
xdebug("cmd",$cmd);
shell_exec($cmd);
//************************************************************************************************************************************
//copy d:\tdw\tdw\auto\bloomberg\temp\2007-03-20___e74b6353.zip \\bucknypdc3\compliance$\BloombergArchive\PostAutomation\2007-03-20
