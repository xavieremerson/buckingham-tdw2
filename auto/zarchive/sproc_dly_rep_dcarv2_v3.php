<?
  ...
  include('../includes/global.php');
	...
	//$trade_date_to_process = '2006-12-06';
...
$arr_dateval = explode(' ',$val);
  if ($arr_dateval[1]."-".$arr_dateval[0] == $date_match_val){ 
   if (!in_array($key,$arr_recent_mri_trade_found)) { 
	 //echo  $key.">>".$val."<br>";
	 $str_show_no_mri_action .= ", <u>". $key . "</u> (". ucwords(strtolower(get_company_name($key))) . ")";
	 }
	}
}
$str_show_no_mri_action = substr($str_show_no_mri_action, 1, 5000);
		...
		$data_to_html_file .= '
			...
			while($row_trades = mysql_fetch_array($result_trades))
			{
				...
				*/ 
				...
				$data_to_html_file .=	'
											<tr>
												<td><br><font color="#000000" size="3" face="Courier"><strong>
												Symbols with MRI Date '.$date_match_val.' and no trades.</strong><br>
												'.$str_show_no_mri_action.'
												</font></td>
											</tr>';
