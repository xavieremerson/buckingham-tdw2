...
//====================================================================================================
	...
	$trade_date_to_process = '2007-02-14';
       ...
       //combine two arrays (this creates a new index)
		...
		$data_to_html_file .= '
			...
			foreach ($sorted_merged_array_trades as $key => $val) {
			 ...
			 if ($rec_to_process[0]=='PLCE') {
			 xdebug("Symbol",$rec_to_process[0]);
			 xdebug("Client",$rec_to_process[1]);			 
			 }
				...
				substr($rec_to_process[2],0,2) != '09' && $rec_to_process[6] == '0.0' && in_array(trim($show_trad_advisor_name),$arr_acct_emp)) {
					...
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
			 		if ($rec_to_process[0]=='PLCE') { echo ">>>>>>>>>>>>>>>>>>>>>>>> HERE1 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<br>"; }
				} elseif (substr($rec_to_process[2],0,2) == '09' and $rec_to_process[11] == 'PDS') { 
					$rowcolor = "0000ff";
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
			 		if ($rec_to_process[0]=='PLCE') { echo ">>>>>>>>>>>>>>>>>>>>>>>> HERE2 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<br>"; }
				} elseif (trim($show_trad_advisor_name) == 'BUCK CAPITAL MGMT') {
					$rowcolor = "0000ff";
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
			 		if ($rec_to_process[0]=='PLCE') { echo ">>>>>>>>>>>>>>>>>>>>>>>> HERE3 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<br>"; }
				} elseif ($rec_to_process[8] == '#outside_employee_trade#') {
					$rowcolor = "0000ff";
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
			 		if ($rec_to_process[0]=='PLCE') { echo ">>>>>>>>>>>>>>>>>>>>>>>> HERE4 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<br>"; }
				}  else {
					$rowcolor = "000000";
					$rowstrongstart = "";
					$rowstrongend = "";
			 		if ($rec_to_process[0]=='PLCE') { echo $rec_to_process[11].">>>>>>>>>>>>>>>>>>>>>>>> HERE5 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<br>"; }
				}
