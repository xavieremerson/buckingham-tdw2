...
////SECTION: WHAT DAY TO PROCESS?
	...
	//$trade_date_to_process = '2008-12-30';
...
////SECTION: Create array of stocks in BCM WatchList for lookup
	$qry_bcm = "SELECT DISTINCT (bcm_cusip) as bcm_val
											FROM bcm_watchlist 
											WHERE bcm_datetime_stop > '".trade_date_to_process." 00:00:00' 
											OR bcm_datetime_stop = '2099-12-31'";
...
////SECTION: Functions
	...
	while ( $row_oth_trades = mysql_fetch_array($result_oth_trades) ) 
	{
		...
		if (stripos(trim($row_oth_trades["oth_symbol"]), "+")!==false) { // or stripos($rec_to_process[0], " ")!==false
			...
			$arr_get_option_detail = gOpt(trim($row_oth_trades["oth_symbol"]));
			if ($arr_get_option_detail[0] != "") {
				//xdebug("Symbol", trim($row_oth_trades["oth_symbol"]).">>".$arr_get_option_detail[0].">>".$arr_get_option_detail[1]);
			  ob_flush();
			  flush();
				$var_bcm_ticker = $arr_get_option_detail[0];
			} else {
				//xdebug("Symbol", trim($row_oth_trades["oth_symbol"]).">>"."ERROR");
			  ob_flush();
			  flush();
				$var_bcm_ticker = "???";
			}
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	$qry_fid_trades = "SELECT 
												...
												a.is_active, 
												substr(concat(trim(a.first_name),' ',trim(a.middle_name),' ',trim(a.last_name)),1,20) as Fullname
											FROM fidelity_emp_trades	 a
											WHERE a.trade_date = '".$trade_date_to_process."'
											AND a.is_active  = 1
											GROUP BY acct_num, symbol";
...
//show_array($arr_acct_emp);
	     ...
	     $result_nfs_trades = mysql_query($query_nfs_trades) or die (tdw_mysql_error($query_nfs_trades));
				while ( $row_nfs_trades = mysql_fetch_array($result_nfs_trades)) 
				{
					...
					if ($row_nfs_trades["trad_advisor_code"] == 'BUCK' and stripos(trim($row_nfs_trades["trad_symbol"]), "+")!==false) { // or stripos($rec_to_process[0], " ")!==false
						$arr_get_option_detail = array();
						$arr_get_option_detail = gOpt(trim($row_nfs_trades["trad_symbol"]));
						if ($arr_get_option_detail[0] != "") {
							//xdebug("Symbol", trim($row_oth_trades["oth_symbol"]).">>".$arr_get_option_detail[0].">>".$arr_get_option_detail[1]);
							$var_bcm_ticker = $arr_get_option_detail[0];
						} else {
					...
					//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					$show_bcm_watch = "";
					if (in_array($var_bcm_ticker,$arr_bcm) and $row_nfs_trades["trad_advisor_code"] == 'BUCK') {
						$count_bcm_watchlist = $count_bcm_watchlist + 1;
						$str_acv = "BCM_WATCH";
					}
	     ...
	     //xdebug("total_count_acv_events",$total_count_acv_events);
			 ...
			 $report_header_string1 = '<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			 															<strong>BCM Watch List ACTIVITY ALERT: 0</strong>
																 </font>';
			 } else {
			 $report_header_string1 = '<font color="#FF00FF" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			 															<strong>BCM Watch List ACTIVITY ALERT: '.$count_bcm_watchlist.'</strong>
																 </font>';
			 }
			...
			foreach ($sorted_merged_array_trades as $key => $val) {
 			 ...
 			 $rec_to_process = explode("^",$val);
				...
				substr($rec_to_process[2],0,2) != '09' && $rec_to_process[6] == '0.0' && in_array(trim($show_trad_advisor_name),$arr_acct_emp)) {
					...
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
				} elseif ($rec_to_process[8] == 'BCM_WATCH' or $rec_to_process[10] == 'BCM_WATCH') {
					$rowcolor = "000000";
					$rowstrongstart = "<strong>";
					$rowstrongend = "</strong>";
...
//				
        ...
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				...
				if (stripos($rec_to_process[0], "+")!==false) { // or stripos($rec_to_process[0], " ")!==false
					$arr_get_option_detail = array();
					$arr_get_option_detail = gOpt($rec_to_process[0]);
					if ($arr_get_option_detail[0] != "") {
						xdebug("Symbol", $rec_to_process[0].">>".$arr_get_option_detail[0].">>".$arr_get_option_detail[1]);
						$var_show_ticker = $rec_to_process[0]."[".$arr_get_option_detail[0]."]";
					} else {
				...
				$data_to_html_file .=	'
											<table width="670" border="1" cellspacing="0" cellpadding="1" bordercolor="#CCCCCC">
												...
												<tr>
													...
													<font color="#000000" size="2" face="Courier"><strong>BCM Watch List Trades</strong></font>
													</td>
												</tr>
												<tr>
													<td colspan="2">
													<font color="#000000" size="3" face="Courier"><strong>f: Employee Trades (Fidelity)</strong><br></font>
													</td>
												</tr>
											</table>';
