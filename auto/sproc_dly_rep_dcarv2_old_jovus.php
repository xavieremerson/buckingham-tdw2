...
////SECTION: WHAT DAY TO PROCESS? 
	...
	$trade_date_to_process = '2009-10-06';
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    //Found Initiation of Coverage
		$ms_qry_ioc = "SELECT dbo.ExchangeSecurities.Ticker as CUSIP, 
										...
										AND dbo.Products.Title like  '%nitiating%'
		...
		while ($row_mri = mssql_fetch_array($ms_results_mri)) {
					...
					if ($rating_change == "Increase") {
					  $img_show = '<img src="images/themes/standard/arrow_up.gif" border="0">';
						$arr_mri[$v_count_mri] = $symbol."<###>".$mri_date."<###>".$rating."<###>".$rating_change."<###>".''."<###>".$target."<###>".$rating_previous;
   ...
   $arr_mri = array();
	 ...
	 //INSERT INTO table
	 $result = mysql_query("truncate table _jovus_migration_mri") or die(mysql_error());
	 foreach($arr_mri as $key=>$value) {
	 		$result = mysql_query("insert into _jovus_migration_mri values(NULL,'".$value."',now())") or die(mysql_error());
	 }
   ...
   //show_array($arr_mri_symbols);
	 ...
	 foreach($arr_mri as $key=>$value) {
						 ...
						 if (date('Y-m-d', strtotime($arr_data[1])) == $trade_date_to_process) {
							 ...
							 if ($str_x_rating != $str_x_rating_previous && trim($str_x_rating_previous) != "") {
							 		$arr_recent_mri[$str_x_symbol] = $str_x_symbol;
							 }
							 
							 //Condition 2: Target Current and Previous are different
							 if ($str_x_target != $str_x_target_next && trim($str_x_target_next) != "") {
							 		$arr_recent_mri[$str_x_symbol] = $str_x_symbol;
							 }
...
$cmd = "copy d:\\tdw\\tdw\\data\\compliance\\".$file_pdf_name. " \\\\bucksnapNY\\SHARE1\\TDW_ComplianceReports\\".$file_pdf_name;
xdebug("cmd",$cmd);
shell_exec($cmd);
//************************************************************************************************************************************
