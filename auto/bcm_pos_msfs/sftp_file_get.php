...
$sftp = new Net_SFTP('sftp.morganstanley.com');
if (!$sftp->login('nuvabcam', 'Qjpz6ZmyUoRB3qZ+')) {
    exit('Login Failed');
}
...
$arr_file_upload_to_db = array(); 


foreach($arr_list as $k=>$filename) {

	$arr_file_upload_to_db[] = $filename;
	//$str_substr_filename = substr($filename, 13, 6);
	//echo $str_substr_filename."\n";

	$sftp->get($filename, "./data_files/".$filename);

}

//print_r($arr_file_upload_to_db);

$file_path = ".\\data_files\\";
$arr_files = array();
foreach (glob($file_path."*.csv") as $filename) {
    $arr_files[] = substr($filename,13,200 );
}
