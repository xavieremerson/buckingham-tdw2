...
//****************************************************************************************************************
//****************************************************************************************************************
//create a map of bm ids and tdw ids. and email

$qry="select ID, bm_id, Email from users_bmatrix_map where ID is not NULL order by bm_id";
$result = mysql_query($qry);
$arr_bm_id = array();
$arr_bm_email = array();
while ($row=mysql_fetch_array($result)) {
$arr_bm_id[$row["bm_id"]] = $row["ID"];
$arr_bm_email[$row["bm_id"]] = $row["Email"];
}

show_array($arr_bm_email);
xdebug(">>>",$arr_bm_email[2]);
echo $arr_bm_email[2];
//show_array($arr_bm_id);
...
foreach ($bm_data->People as $People) {
    ...
    $analyst_id = $People["id"];
    echo $analyst_id."<br>";

		foreach($People->Security as $Security) {
		
			$symbol = $Security["pricingSymbol"];
		
			//check if it is already covered.
			$count_exists = db_single_val("select count(*) as single_val from acv_analyst_coverage
			                               where acv_symbol = '".$symbol."' 
																		 and acv_email = '".$arr_bm_email[$analyst_id]."'");
			
			if ($count_exists > 0) {
			  $str_qry = "UPDATE warehouse.acv_analyst_coverage SET acv_last_updated = CURDATE( ) 
															where acv_symbol = '".$symbol."' 
															and acv_email = '".$arr_bm_email[$analyst_id]."'";
				$result = mysql_query($str_qry); // or die(tdw_mysql_error($str_qry));
				xdebug("arr_bm_email[$analyst_id]",$arr_bm_email[$analyst_id]);
			
			} else {
				
				$str_qry = "INSERT INTO acv_analyst_coverage (
														auto_id ,	acv_symbol ,acv_email ,acv_latest_date ,
														acv_tdw_userid ,acv_tdw_rr_num ,acv_last_updated) 
														VALUES (
														NULL , '".$Security["pricingSymbol"]."', '".$arr_bm_email[$analyst_id]."', CURDATE( ) , '".$arr_bm_id[$analyst_id]."', 'NA', CURDATE( ))";
				echo $str_qry."<br>";
				$result = mysql_query($str_qry); // or die(tdw_mysql_error($str_qry));
				xdebug("arr_bm_email[$analyst_id]",$arr_bm_email[$analyst_id]);
			}															 
    	...
    	echo ">>".$Security["pricingSymbol"].">>". $Security["companyName"].">>".$analyst_id."<br>";
		}
...
/*INSERT INTO warehouse.acv_analyst_coverage (
auto_id ,
acv_symbol ,
acv_email ,
acv_latest_date ,
...
VALUES (
NULL , 'ZZZ', 'zzxcxzxczxczxczxczczc', CURDATE( ) , '12', 'YYY', '2013-06-27'
);*/
...
echo "ANALYST COVERAGE UNIVERSE CREATED.";
exit;
