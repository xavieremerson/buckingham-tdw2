...
//Create a user map table between tdw ID and blue matrix id based on email.
$result = mysql_query("truncate table users_bmatrix_map");
...
foreach ($bm_data->People as $People) {
		...
		$qry_users = "INSERT INTO `warehouse`.`users_bmatrix_map` (
								`ID` ,
								`bm_id` ,
								`Firstname` ,
								`Lastname` ,
								...
								VALUES (
								NULL, '".$People->id."', '".mysql_real_escape_string($People->firstName)."', '".
																					 mysql_real_escape_string($People->lastName)."', '".
																					 mysql_real_escape_string($People->initial)."', '".$People->email."')";
		 $result = mysql_query($qry_users) or die(tdw_mysql_error($qry_users));

		}

}

$result = mysql_query("UPDATE users_bmatrix_map a
											LEFT JOIN users b 
													ON lower(a.email) = lower(b.email)
											SET a.ID = b.ID");

echo "USER MAP CREATED.";
exit;
