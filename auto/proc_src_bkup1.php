						  ...
						  //get only regular trades, not the cancelled trades, the cancelled trades will be 
							...
							while($row_cancelled_trades = mysql_fetch_array($result_cancelled_trades))
							{
								...
								while($row_get_original_trade = mysql_fetch_array($result_get_original_trade))
								{
									...
									//this DOES NOT WORK because NFS puts the account short name without the & sign when trades are entered
									// intead using lookup against the nfs_nadd table 
								  ...
								  //xdebug("ot_comm_advisor_code",$ot_comm_advisor_code);
									...
									//GOTTO FIX THIS LOGIC, IF DATE IS LAST DATE OF MONTH OR QUARTER OR YEAR PROCESS DIFFERENTLY, for now >=
  								 ...
  								 //xdebug("query_update_cancel_b",$query_update_cancel_b);
									 ...
									 for ($bd=1; $bd<5; $bd++) {
											...
											$is_same_year_bd = 	  sameyear($process_cancel_for_date,$ot_comm_trade_date);
											$is_same_month_bd = 	samemonth($process_cancel_for_date,$ot_comm_trade_date);
											$is_same_qtr_bd = 		sameqtr($process_cancel_for_date,$ot_comm_trade_date);
