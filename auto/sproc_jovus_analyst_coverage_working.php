...
$ms_qry_asym = "SELECT dbo.Products.ProductID, dbo.Issuers.CUSIP, dbo.Persons.WorkEmail, left(convert(varchar(20),dbo.Prod_Statuses.DateTime,20),10) as releasedate
								FROM ((((dbo.Products 
...
while ($row_asym = mssql_fetch_array($ms_results_asym)) {
  ...
  */
	...
	$result_exist = mysql_query("SELECT * FROM acv_analyst_coverage 
																where acv_symbol = '".strtoupper(trim($row_asym["CUSIP"]))."'
	...
	$qry_insert = "INSERT INTO acv_analyst_coverage
									...
									VALUES (
									NULL , 
									'".strtoupper(trim($row_asym["CUSIP"]))."', 
			...
			echo strtoupper(trim($row_asym["CUSIP"]))."<br>";
			ob_flush();
			flush();
