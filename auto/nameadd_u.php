...
*/
	...
	echo "from nameadd_u.php trade_date_to_process = [".$trade_date_to_process."]\n";
	...
	while (!feof ($fp)) { 
		...
		//xdebug("recnum",$recnum);
			if ($recnum == "1") {
				...
				$nadd_short_name = tdw_clean_string(substr($content,44,10));
...
"'".trim($nadd_birth_date_shadow)."')";
				...
				//There is problem data in the source files from NFS (Date: 10/12/2007)
				//Caused the import routine to fail. CRITICAL ERROR
				//Hence commenting the die section for now.
				
				//$result = mysql_query($str_sql); //or die(tdw_mysql_error($str_sql));
				$result = mysql_query($str_sql) or tdw_mysql_error_email($str_sql); //or die(tdw_mysql_error($str_sql));
...
nadd_address_line_6 from nfs_nadd_101";
	...
	$result1 = mysql_query("UPDATE nfs_nadd set nadd_advisor = substring(nadd_short_name,1,4) where nadd_branch='PDY'") or die(mysql_error());	 
	$result2 = mysql_query("UPDATE nfs_nadd set nadd_advisor = nadd_short_name where nadd_branch !='PDY'") or die(mysql_error());	 
?>