...
print_r($total_commission);
		...
		{
//print_r($row_comm);
//			$check_r = "SELECT chek_amount FROM  chk_chek_payments_etc WHERE chek_date='".$row_comm["trad_trade_date"]."' AND chek_advisor='".$row_comm["trad_advisor_code"]."'";
//			$check_r = "SELECT chek_amount FROM  chk_chek_payments_etc WHERE chek_date>".$arr_quarter_cal_dates[0]." AND chek_advisor='".$row_comm["trad_advisor_code"]."'";
			$check_r = "SELECT chek_amount FROM  chk_chek_payments_etc WHERE chek_date between '".$arr_quarter_cal_dates[0]."' AND '".$arr_quarter_cal_dates[1]."' AND chek_advisor='".$row_comm["trad_advisor_code"]."'";
			$check = create_arr($check_r);
			$clnt_name = create_arr("select clnt_name from int_clnt_clients where clnt_code = '".$row_comm["trad_advisor_code"]."'");
			...
			?>
						...
						<tr <?=$diff_style?> <? if ($scount % 2) { echo "class='trlight'"; } else { echo "class='trdark'";}?>>
							<td><?=$row_comm["commission"]+0.0?></td>
							<td><?=$row_comm["commission"]+$check[0]?></td>
							<td ><?=$check[0]?></td>
							<td><?=$clnt_name[0]?></td>
							<td>&nbsp;</td>
						</tr>
<?
		$scount = $scount + 1;
		$total_analyst += $row_comm["commission"];
		$total_sales += $row_comm["commission"]+$check[0];
		$total_diff += $check[0];
		}
