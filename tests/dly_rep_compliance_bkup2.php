		...
		$data_to_html_file .= '
			...
			while($row_trades = mysql_fetch_array($result_trades))
			{
				...
				$data_to_html_file .=	'
											...
											<font color="#FF0000" size="3" face="Courier"><strong>Red: Trades in stocks with same day Research/MRI.</strong></font>
											';
...
//$arr_recipient[1] = 'lkarp@buckresearch.com';
//$arr_recipient[2] = 'tsutera@buckresearch.com';
//$arr_recipient[3] = 'jperno@buckresearch.com';

foreach ($arr_recipient as $key => $emailval) {

				//echo get_user_id ($emailval);
				
				$link = $_site_url."repsvr.php?rep=DCAR&src=".rand(10000000,99999999).str_replace('-','N',$trade_date_to_process).str_pad(get_user_id ($emailval),10,'Q',1).md5("pprasad@centersys.com");
				$email_log = '
								<hr>
