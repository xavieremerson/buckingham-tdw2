<?
  ...
  include('includes/global.php');
	...
	$trade_date_to_process = '2006-11-24';
	...
	$qry_oth_trades = "SELECT auto_id,
													...
													oth_price,
													oth_commission,
													(oth_commission/oth_quantity) as centspershare,
													oth_net_money,
													oth_trade_time,
													oth_isactive
	...
	while ( $row_oth_trades = mysql_fetch_array($result_oth_trades) ) 
	{
		...
		$arr_oth_trades_symbol[$row_oth_trades["auto_id"]] = $row_oth_trades["oth_symbol"];
	}	
...
//show_array($arr_acct_emp);
	     ...
	     $result_nfs_trades = mysql_query($query_nfs_trades) or die (tdw_mysql_error($query_nfs_trades));
				...
				{
					$arr_nfs_trades_to_merge[$maint_count_nfs] =  trim($row_nfs_trades["trad_symbol"])."^".
																												$row_nfs_trades["trad_advisor_name"]."^".
																												$row_nfs_trades["trad_rr"]."^".
																												$row_nfs_trades["trad_buy_sell"]."^".
																												$row_nfs_trades["trad_quantity"]."^".
																												$row_nfs_trades["trade_price"]."^".
																												$row_nfs_trades["trad_commission"]."^".
																												$row_nfs_trades["trad_cents_per_share"];
       ...
       //combine two arrays (this creates a new index)
			 ...
			 $merged_array_trades = array_merge($arr_oth_trades_to_merge, $arr_nfs_trades_to_merge);
		...
		$data_to_html_file .= '
			...
			while($row_trades = mysql_fetch_array($result_trades))
			{
				...
				$rowstrongend = "</strong>";
						...
						if (
							substr($row_trades["trad_rr"],0,2) != '09' && $row_trades["trad_commission"] == '0.0' && in_array(trim($show_trad_advisor_name),$arr_acct_emp)) {
							$rowcolor = "0000ff";
							$rowstrongstart = "<strong>";
							$rowstrongend = "</strong>";
				...
				xdebug("Hold Symbol Match",$dummmy);
				//Õ  ß
						...
						//Insert BCM Trades here
						if (in_array(trim($hold_symbol), $arr_oth_trades_symbol)) {
								foreach ($arr_oth_trades_symbol as $key => $value) {
     							//show_array($arr_oth_processed);
									if (in_array($key,$arr_oth_processed)) {
										//do nothing
										echo "Already Processed >> ".$value."<BR>";
									} else {
										//echo $key."<BR>".$value;
										//echo $arr_oth_trades[$key]."<BR>";
										$show_dat = explode('^',$arr_oth_trades[$key]);
										//show_array($show_dat);
										if ($value == trim($hold_symbol)) {
										
											$arr_oth_processed[$processed_count] = $key;
											...
											$data_to_html_file .= '
																 <tr>
																		<td><font color="#'.$rowcolor.'" size="2" face="Courier">'.$rowstrongstart.
																		str_replace(" ","&nbsp;",str_pad('ß',4," ",1)).
																		str_replace(" ","&nbsp;",str_pad('BUCK CAPITAL MANAGEMENT',26," ",1)).
																		str_replace(" ","&nbsp;",str_pad(' ',8," ",1)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[1],10," ",1)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[0],5," ",1)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[2],10," ",0)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[3],8," ",0)).
																		str_replace(" ","&nbsp;",str_pad($show_mri,14," ",0)).
																		str_replace(" ","&nbsp;",str_pad($show_rres,10," ",0)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[4],14," ",0)).
																		str_replace(" ","&nbsp;",str_pad($show_dat[0],6," ",0)).
																		$rowstrongend.'</font></td>
																	</tr>';
											$count_row_trades = $count_row_trades + 1;
										}
				...
				xdebug("Hold Symbol Mismatch",$dummmy);
						$data_to_html_file .= '
											 <tr>
