...
if ($sel_qtr != "" AND $sel_year != "") {
 ...
 //print_r($arr_clnt_rr_initials);
	...
	<?
		...
		$pay_adv_q = "SELECT pay_advisor_code as pac, sum(pay_percent) as total_percent FROM pay_analyst_allocations WHERE pay_year='2013' and  pay_qtr='3' and pay_isactive = 1 
	  ...
	  $result_adv_q = mysql_query($pay_adv_q) or die(tdw_mysql_error($pay_adv_q));
		...
		//show_array($pay_advisor_percent);
		...
		foreach ($pay_advisor_code as $k=>$v) {
			...
			//echo $v ." >> " . $arr_clients[$v] ." >> " . $analyst_payout ." >> " . $sales_payout ." >> " . ($analyst_payout-$sales_payout) . "<br>";

			if ($analyst_payout!=$sales_payout) {$diff_style = "style=\"color: red\"";}
			else {$diff_style = "";}
			?>
