...
//Create Lookup Array of Client Code / Client Name
	$qry_clients = "select clnt_code, max(clnt_name) as clnt_name from int_clnt_clients group by clnt_code";
...
//*********************************************************************************************
//Create Lookup Array of Client Code / Client Reps
	$qry_clients_rr = "select a.clnt_code, a.clnt_rr1, a.clnt_rr2, a.clnt_trader, b.Fullname from int_clnt_clients a 
									left join users b on a.clnt_rr1 = b.Initials
									where b.Initials != ''
									group by a.clnt_code";
	$result_clients_rr = mysql_query($qry_clients_rr) or die (tdw_mysql_error($qry_clients_rr));
	$arr_clients_rr1 = array();
	$arr_clients_rr2 = array();
	while ( $row_clients_rr = mysql_fetch_array($result_clients_rr) ) 
	{
			$arr_clients_rr1[$row_clients_rr["clnt_code"]] = $row_clients_rr["clnt_rr1"];
			$arr_clients_rr2[$row_clients_rr["clnt_code"]] = $row_clients_rr["clnt_rr2"];
	}
//show_array($arr_clients_rr1);							
...
$qry_clnt_for_rr = "SELECT distinct(a.chek_advisor) as chek_advisor
										...
										AND (trim(b.clnt_rr2) = '' or b.clnt_rr2 is NULL)
...
$qry_clnt_for_rr = "SELECT distinct(a.chek_advisor) as chek_advisor
                   ...
                   left join int_clnt_clients_history b on a.chek_advisor = b.clnt_code 
								  WHERE a.chek_date between '".$arr_quarter_cal_dates[0]."' AND '".$arr_quarter_cal_dates[1]."'
									  ...
									  AND a.chek_isactive = 1
										AND (b.clnt_rr2 = '' or b.clnt_rr2 is NULL)
								 ORDER BY a.chek_advisor";
$result_clnt_for_rr = mysql_query($qry_clnt_for_rr) or die (tdw_mysql_error($qry_clnt_for_rr));
while ( $row_clnt_for_rr = mysql_fetch_array($result_clnt_for_rr) ) 
{
