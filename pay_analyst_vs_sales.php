...
if ($sel_qtr != "" AND $sel_year != "") {
	...
	function getsole($initials) {
		...
		$total = 0;
		foreach($arr_clnt_rr_initials as $clnt=>$rr) { 			if ($rr == $initials) {				$total = $total + $arr_composite_primary[$clnt];			}		}
		return $total;
	}
	...
	function getshrd($initials) {
		...
		$total = 0;
		foreach($arr_clnt_rr_initials as $clnt=>$rr) {			if ($rr == $initials) {				$total = $total + $arr_composite_shared[$clnt];			}		}
		return $total;
	}
  ...
  //$arr_comm_for_rr_comm["INTR"] = 1;	
	...
	///\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//\/\/\/\/\/\/\/\/\/\/\
	//ARRAY OF SALES WITH PAYOUT IN REVERSE ORDER.
	$arr_initials_payout = array();
	foreach($arr_initials_id as $k=>$v) {
		$total = getsole($k) + getshrd($k);
		$arr_initials_payout[$k] = $total;
	} 
	asort($arr_initials_payout);
	//show_array($arr_initials_payout);
	$z=0;
	$arr_tmp = array();
	foreach($arr_initials_payout as $k=>$v) {
		$arr_tmp[$z] = $k."|".$v;
		$z++;
	}
	
	$arr_initials_payout_rsort = array();
	for ($n=0;$n<$z;$n++) {
		$arr_vals = explode("|",$arr_tmp[$z-$n-1]);
		$arr_initials_payout_rsort[$arr_vals[0]] = $arr_vals[1];
	}

	//show_array($arr_initials_payout_rsort);
	//echo $z;
	//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//\/\/\/\/\/\/\/\/\/\/\
	//ARRAY OF ANALYSTS IN REVERSE ORDER
	$arr_analysts = create_arr("select ID as k, concat(Lastname, ', ', Firstname) as v from users WHERE Role = 1 and user_isactive = 1", 2);
	asort($arr_analysts);
	
	//POPULATE LARGE ARRAY WITH DATA
	$arr_large_payout = array();  //sales//analyst//client//percent//total$//
	//pay_qtr  pay_year  pay_final  pay_sales_id  pay_advisor_code  pay_analyst_id  pay_percent  pay_lastedited  pay_lastedited_by  pay_isactive 
	$qry_payout = "select * 
									from pay_analyst_allocations 
									where pay_qtr = '".$sel_qtr."'
									and pay_year = '".$sel_year."'";
	$result_payout = mysql_query($qry_payout) or die (tdw_mysql_error($qry_payout));
	while ( $row_payout = mysql_fetch_array($result_payout) ) 
	{
		$arr_large_payout[$row_payout["pay_sales_id"]][$row_payout["pay_advisor_code"]][$row_payout["pay_analyst_id"]] = $row_payout["pay_percent"];
	}

	foreach($arr_initials_payout_rsort as $in=>$sid) {
		echo $in.">".$sid."<br>";
		//get total for analyst against all clients			
		foreach($arr_analysts as $aid=>$name) {
