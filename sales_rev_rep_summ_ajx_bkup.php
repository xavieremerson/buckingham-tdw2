...
//Client current tier and how many years in that tier and if it went up or down.
  ...
  foreach ($arr_active_clients as $k=>$v) {
	...
	$qry_cymtd_chk = "select chek_advisor, round(sum(chek_amount),0) as clnt_revenue
													from chk_chek_payments_etc  
													where chek_date between '".date('Y',strtotime($trade_date_to_process))."-".date('m',strtotime($trade_date_to_process))."-01' and '".$trade_date_to_process."' 
													and chek_isactive = 1
													and chek_advisor in ".$str_active_clients."
													group by chek_advisor
													order by chek_advisor";
		...
		foreach($arr_sole_joint as $dk=>$dv) {
			foreach($arr_tier_val as $ek=>$i) {
				...
				if ($arr_tier_count[$ck][$dv][$i]) {
							...
							$cymtd = rev_group_clients($arr_merge_cymtd, $arr_tier_count[$ck][$dv][$i][1]);
							$pymtd = rev_group_clients($arr_merge_pymtd, $arr_tier_count[$ck][$dv][$i][1]);
							if ($pymtd > 0) { 
							...
							$cyytd = rev_group_clients($arr_merge_cyytd, $arr_tier_count[$ck][$dv][$i][1]);
							$pyytd = rev_group_clients($arr_merge_pyytd, $arr_tier_count[$ck][$dv][$i][1]);
							if ($pyytd > 0) { 
							...
							$py_full = rev_group_clients_past_years ($arr_clnt_yearly_total_process, date('Y',strtotime($trade_date_to_process))-1, $arr_tier_count[$ck][$dv][$i][1]);
							$ppy_full = rev_group_clients_past_years ($arr_clnt_yearly_total_process, date('Y',strtotime($trade_date_to_process))-2, $arr_tier_count[$ck][$dv][$i][1]);
							
							if ($dv == "Sole") { $str_type = "PRIMARY";} elseif ($dv == "JP") { $str_type = "Shared Pri."; } elseif ($dv == "JS") { $str_type = "Shared Sec."; } else { $str_type = "";}
														
							?>
              <tr class="<?=$class_row?>"> 
              	<td><img src="./images/lf_v1/expand.png" border="0" alt="Expand" /></td>
							<? echo "<td>".$i."</td>"; ?>
                <td><?=$arr_reps_list_array [$ck][2]?></td>
                <td><?=$str_type?></td>
                <td align="right"><?=$arr_tier_count[$ck][$dv][$i][0]?></td>
