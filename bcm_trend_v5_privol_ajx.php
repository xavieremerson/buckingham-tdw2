...
/*	105^SCS^P = [09/22/2011^SCS^398,800^Sell^5.530000^-24.95^6]
  ...
  108^SCS^N = [09/22/2011^SCS^398,800^Sell^5.530000^Steelcase Inc.'s (NYSE:SCS) shares fell 18% a day after the office-furniture maker reported a second-quarter profit below analysts' consensus projection. ]*/
	...
	if ($mode=="download" && $dtype=="summary") {
			...
			$str_out_html = "";
			$str_out_html .= '<html xmlns="http://www.w3.org/1999/xhtml">
						<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head>
						<body>';
		
			$str_out_html .= '<a href="'.$PHP_SELF.'?user_id='.$user_id.'&mode=download&dtype=summary&sel_month='.$sel_month.'" class="ilt" target="_blank">Download Summary</a>&nbsp;&nbsp;&nbsp;
												<a href="'.$PHP_SELF.'?user_id='.$user_id.'&mode=download&dtype=detail&sel_month='.$sel_month.'" class="ilt" target="_blank">Download Details</a><br>
					...
					<table style="border-bottom-style:inset; border-bottom-width:thin" border="1" cellspacing="0" cellpadding="2">
					<tr>
							<td colspan="5"><strong>Monthly Price & Volume Review ' . $str_report_header . '</strong></td>
					</tr>
					<tr style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px; color:#000099">
