<?
			...
			*/
			foreach($arr_master_new as $k=>$v) {
				if (substr($k,0,1) == '0') {
					$qry = "insert into mry_tmp_process(val) values('".get_userid_for_rr($k)."')";
					//xdebug("qry"."/".$k,$qry);
					$result_insert = mysql_query($qry) or die (tdw_mysql_error($qry));
				}
			}			
  		...
  		//CAPTURE SUBTOTAL ROW IN ARRAY TO GET GRAND TOTAL
						...
						$data = explode("^",$v);
							if ($data[1] == $row_users["ID"]) {
									...
									if (trim($data[3]) != "") {
										...
										$str_name_for_excel = get_user_by_id($data[1]);
										$wks->write($count_row_xls, 3, $str_name_for_excel, $format_data_3);
									}
									...
									*/
									...
									$wks->writeFormula($count_row_xls, 18, "=".$arr_xl_cols[10].($count_row_xls+1).
																												 "+".$arr_xl_cols[11].($count_row_xls+1).
																												 "+".$arr_xl_cols[12].($count_row_xls+1).
																												 "+".$arr_xl_cols[13].($count_row_xls+1).
																												 "+".$arr_xl_cols[14].($count_row_xls+1).
																												 "+".$arr_xl_cols[15].($count_row_xls+1), $format_currency_1);
									if ($data[11] != 0) {
										$wks->writeNumber($count_row_xls, 19, $data[11], $format_currency_1);
									}
									$wks->writeFormula($count_row_xls, 20, "=".$arr_xl_cols[18].($count_row_xls+1)."+".$arr_xl_cols[19].($count_row_xls+1), $format_currency_1);
									$wks->writeString($count_row_xls, 25, $row_users["lastname"]);
									...
									//$wks->writeFormula($count_row_xls, 12, "=sum(".$arr_xl_cols[12]."4".":".$arr_xl_cols[12].($count_row_xls).")", $format_currency_2);
									$wks->writeFormula($count_row_xls, 18, "=sum(".$arr_xl_cols[18]."4".":".$arr_xl_cols[18].($count_row_xls).")", $format_currency_2);
			...
			//SECTION 2 START
			$qry_users = "SELECT a.ID, a.lastname, a.Role, a.Fullname as Fullname
										FROM Users a, mry_tmp_process b
										WHERE a.ID = b.val
										AND (a.Role = 4)
										AND (a.ID != 288)
					...
					foreach($arr_master_new as $k=>$v) {
						$data = explode("^",$v);
							if ($data[1] == $row_users["ID"]) {
									//show_array($data);
									$wks->writeString($count_row_xls, 2, $data[0]);
									$wks->write($count_row_xls, 3, $row_users['Fullname'],$format_data_3);  //$data[3]
									$wks->writeNumber($count_row_xls, 5, $data[4],$format_currency_1);
									...
									*/
									...
									//$wks->writeFormula($count_row_xls, 12, "=sum(".$arr_xl_cols[12].$hold_count_row_xls.":".$arr_xl_cols[12].($count_row_xls).")", $format_currency_2);
									$wks->writeFormula($count_row_xls, 18, "=sum(".$arr_xl_cols[18].$hold_count_row_xls.":".$arr_xl_cols[18].($count_row_xls).")", $format_currency_2);
						...
						$data = explode("^",$v);
							if ($data[1] == $row_users["ID"]) {
									...
									*/
									...
									//$wks->writeFormula($count_row_xls, 12, "=sum(".$arr_xl_cols[12].$hold_count_row_xls_new.":".$arr_xl_cols[12].($count_row_xls).")", $format_currency_2);
									$wks->writeFormula($count_row_xls, 18, "=sum(".$arr_xl_cols[18].$hold_count_row_xls_new.":".$arr_xl_cols[18].($count_row_xls).")", $format_currency_2);
			...
			//SECTION 4 BEGIN
					foreach($arr_master_new as $k=>$v) {
						$data = explode("^",$v);
							if (substr($data[0],0,1) != '0' AND substr($data[0],0,1) != '4') {
									//show_array($data);
									$wks->writeString($count_row_xls, 2, $data[0]);
									$wks->write($count_row_xls, 3, $data[3],$format_data_3);
									$wks->writeNumber($count_row_xls, 5, $data[4],$format_currency_1);
									...
									*/
									...
									$wks->writeString($count_row_xls, 25, "");
									...
									//$wks->writeFormula($count_row_xls, 12, "=sum(".$arr_xl_cols[12].$hold_count_row_xls_new_2.":".$arr_xl_cols[12].($count_row_xls).")", $format_currency_2);
									$wks->writeFormula($count_row_xls, 18, "=sum(".$arr_xl_cols[18].$hold_count_row_xls_new_2.":".$arr_xl_cols[18].($count_row_xls).")", $format_currency_2);
			...
			//SECTION 5 BEGINS
			//Array for 400 Accounts
			$arr_400 = array();
			$arr_400['401'] = "O'Gorman/Maguire/Donner";
					foreach($arr_master_new as $k=>$v) {
						$data = explode("^",$v);
							if (substr($data[0],0,1) == '4') {
									//show_array($data);
									$wks->writeString($count_row_xls, 2, $data[0]);
									$wks->write($count_row_xls, 3, $arr_400[$data[0]],$format_data_3); //$data[3]
									$wks->writeNumber($count_row_xls, 5, $data[4],$format_currency_1);
			...
			$wks->writeFormula($hold_count_row_xls_new_3, 5,  "=".$arr_xl_cols[5].($arr_subtotal_row[0]+1).
																												...
																												"+".$arr_xl_cols[5].($arr_subtotal_row[3]+1).
																												"+".$arr_xl_cols[5].($arr_subtotal_row[4]+1), $format_currency_2);
			$wks->writeFormula($hold_count_row_xls_new_3, 6,  "=".$arr_xl_cols[6].($arr_subtotal_row[0]+1).
																												...
																												"+".$arr_xl_cols[6].($arr_subtotal_row[3]+1).
																												"+".$arr_xl_cols[6].($arr_subtotal_row[4]+1), $format_currency_2);
			$wks->writeFormula($hold_count_row_xls_new_3, 8,  "=".$arr_xl_cols[8].($arr_subtotal_row[0]+1).
																												...
																												"+".$arr_xl_cols[8].($arr_subtotal_row[3]+1).
																												"+".$arr_xl_cols[8].($arr_subtotal_row[4]+1), $format_currency_2);
			$wks->writeFormula($hold_count_row_xls_new_3, 10, "=".$arr_xl_cols[10].($arr_subtotal_row[0]+1).
																												...
																												"+".$arr_xl_cols[10].($arr_subtotal_row[3]+1).
																												"+".$arr_xl_cols[10].($arr_subtotal_row[4]+1), $format_currency_2);
...
*/																												
			$wks->writeFormula($hold_count_row_xls_new_3, 18, "=".$arr_xl_cols[18].($arr_subtotal_row[0]+1).
																												"+".$arr_xl_cols[18].($arr_subtotal_row[1]+1).
																												"+".$arr_xl_cols[18].($arr_subtotal_row[2]+1).
																												"+".$arr_xl_cols[18].($arr_subtotal_row[3]+1), $format_currency_2);
...
show_array($arr_debug_info);
echo "<br />master_arr_commissions<br /><br />";
show_array($master_arr_commissions);
echo "<br />master_arr_commission_v1<br /><br />";
show_array($master_arr_commission_v1);
echo "<br />master_arr_commission_v2<br /><br />";
show_array($master_arr_commission_v2);
echo "<br />master_arr_commission_v3<br /><br />";
show_array($master_arr_commission_v3);
echo "<br />master_arr_commission_v4<br /><br />";
show_array($master_arr_commission_v4);
//echo "arr_master_new<br /><br />";
