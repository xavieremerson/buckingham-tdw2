...
function getComment(cid)
{
	...
	if ($(btnid).src == 'http://192.168.20.63/tdw/images/lf_v1/collapse_b.png') {
		...
		var url = 'http://192.168.20.63/tdw/client_master_ajx_ajx.php';
		...
		pars = pars + '&xrand=' + ran_number;
			...
			(
				...
				{     
					...
					onFailure: 
					function(){ showdebug('Error accessing Preapproval Requests.') }
				}
			);	
	}
...
if ($req_ajax) {
	$query_clients = "SELECT * from int_clnt_clients where ".$str_show_deleted." and clnt_name like '".$strltr."%' ". $strrep . $strtrdr. $strtype." order by clnt_name";
	$query_money = "SELECT clnt_code from int_clnt_clients where ".$str_show_deleted." and clnt_name like '".$strltr."%' ". $strrep . $strtrdr. $strtype. " order by clnt_name";
} else {
	$query_clients = "SELECT * from int_clnt_clients where clnt_isactive != 0 order by clnt_name";
	$query_money = "SELECT clnt_code from int_clnt_clients where clnt_isactive != 0 order by clnt_name";
}
			...
			<tr>
              ...
              if ($mqy_sel == 'M') {
                ...
                echo date('Y').' YTD';
						...
						$str_address = "";
						$str_address .= $row["clnt_address_line_1"]."<br>";
						if (trim($row["clnt_address_line_2"]) != "") {
							$str_address .= $row["clnt_address_line_2"]."<br>";
						}
						if ($row["clnt_address_city"]!="" && $row["clnt_address_state"] !="") {
							$str_address .= $row["clnt_address_city"].", ".$row["clnt_address_state"]." ".$row["clnt_address_postal_code"];
						}
								...
								if ($process_tier == 1) {
									...
									if ($row["clnt_status"] == 'A') {
											...
											if ($val_tier == $valcriteria) {
															...
															?>
																	<tr <?=$rowclass?>>
																	<td nowrap>&nbsp; <a href="client_master_ajx.php?type=manage&action=remove&ID=<?=$row["clnt_auto_id"]?>&user_id=<?=$val_user_id?>" 
																		 onclick="javascript:return confirm('Are you sure you want to remove <?=trim(str_replace("'","",$row["clnt_name"]))?> from the list?')">
																		 <img src="images/themes/standard/delete.gif" alt="Delete"></a>&nbsp; </td>
																	<td nowrap>&nbsp; <a href="javascript:CreateWnd('client_master_edit.php?user_id=<?=$val_user_id?>&ID=<?=$row["clnt_auto_id"]?>', 650, 630, false);"> 
																	<img src="images/themes/standard/edit.gif" alt="Edit"></a>&nbsp; </td>
																	<td> </td>
																	<td>&nbsp;<?=trim($row["clnt_name"])?></td>
																	<td>&nbsp;<?=$row["clnt_code"]?></td>
																	<td>&nbsp;<?=$row["clnt_large_trader_id"]?></td>
																	<td>&nbsp;<?=$row["clnt_rr1"]?></td>
																	<td>&nbsp;<?=$row["clnt_rr2"]?></td>
                                  ...
                                  <td><?=$str_address?></td>
															...
															?>
																	...
																	<td> </td>
																	<td> </td>
																	<td nowrap>&nbsp; <a href="client_master_ajx.php?type=manage&action=undelete&ID=<?=$row["clnt_auto_id"]?>&user_id=<?=$val_user_id?>"  
																		 onclick="javascript:return confirm('Are you sure you want to ADD BACK <?=trim(str_replace("'","",$row["clnt_name"]))?> to the list?')">
																		 <img src="images/plus14.png" alt="Add Back"></a>&nbsp; </td>
																	<td>&nbsp;<?=trim($row["clnt_name"])?></td>
																	<td>&nbsp;<?=$row["clnt_code"]?></td>
																	<td>&nbsp;<?=$row["clnt_large_trader_id"]?></td>
																	<td>&nbsp;<?=$row["clnt_rr1"]?></td>
