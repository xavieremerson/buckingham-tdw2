...
ini_set("display_errors", 0); 
  ...
  include('includes/global.php');
	...
	include('auto/sproc_alert_bcm_vol_pct_func.php');  
	
	include("bcm_trend_v5_privol_ajx_holding_inc.php");
   ...
   return date('Y-m-d', $result);
	...
	foreach ($arr_symbols as $k=>$valsymbol) {
		...
		foreach($arr_price_percent as $k=>$v) {
			...
			if ($val_rows > 0 || 1==1) { //<a class="ilt">SYMBOL: </a>$valsymbol<br>
...
/*	105^SCS^P = [09/22/2011^SCS^398,800^Sell^5.530000^-24.95^6]
  ...
  108^SCS^N = [09/22/2011^SCS^398,800^Sell^5.530000^Steelcase Inc.'s (NYSE:SCS) shares fell 18% a day after the office-furniture maker reported a second-quarter profit below analysts' consensus projection. ]*/
	...
	if ($mode=="download" && $dtype=="summary") {
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			//This link to be revisited upon meeting with New Team.
			...
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	} else if ($mode=="download" && $dtype=="detail") {
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			...
			/*	105^SCS^P = [09/22/2011^SCS^398,800^Sell^5.530000^-24.95^6]
				...
				108^SCS^N = [09/22/2011^SCS^398,800^Sell^5.530000^Steelcase Inc.'s (NYSE:SCS) shares fell 18% a day after the office-furniture maker reported a second-quarter profit below analysts' consensus projection. ]*/
						...
						if ($arr_index_k[2] == 'P') {
							if ( ( ($arr_index_v[3] == 'Sell' || $arr_index_v[3] == 'Short') && $arr_index_v[5] < 0) || ( ($arr_index_v[3] == 'Buy' || $arr_index_v[3] == 'Cover') && $arr_index_v[5] > 0)) {
								$str_xl .= '<tr><td colspan="10"></td></tr><tr class="ilt">
																			<td><strong>'.$arr_index_k[1].'</strong></td>
																			<td>Price</td>
																			<td>'.$arr_index_v[0].'</td>
																			<td><strong>'.get_company_name($arr_index_k[1]).'</strong></td>
																			<td>'.$arr_index_v[2].'</td>
																			<td>'.$arr_index_v[3].'</td>
																			<td>'.$arr_index_v[4].'</td>
																			<td>'.$arr_index_v[5].'%</td>
								...
								//Show the Holdings Data in 5 columns.
								$v = format_date_mdy_to_ymd($arr_index_v[0]);
								$valsymbol = $arr_index_k[1];
								$str_xl .= get_holdings_excel($v, $valsymbol);
			...
			$str_out_html .= '<a href="'.$PHP_SELF.'?user_id='.$user_id.'&mode=download&dtype=detail&sel_month='.$sel_month.'" class="ilt" target="_blank">Download (Microsoft Excel Format)</a><br>
												<font style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:9px; color:#ff0000">* Note: Processing downloads may take upto 30 seconds</font><br><br>';
												
			$str_out_html .= '*NOTE: The following image denotes the presence of the relevant stock symbol on the CITTA List.<br>
											 <img src="images/t1_t5.png" border="0"><br><br>';

      //???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????

			$str_out_html .= '<table style="border-bottom-style:inset; border-bottom-width:thin" border="1" cellspacing="0" cellpadding="2">
					<tr>
							<td colspan="10"><strong>Monthly Price & Volume Review (With Holdings T-0 through T-4) ' . $str_report_header . '</strong></td>
					</tr>
					<tr style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px; color:#000099">
							<td><strong>Symbol</strong></td>
							<td><strong>Criteria</strong></td>
			...
			/*	105^SCS^P = [09/22/2011^SCS^398,800^Sell^5.530000^-24.95^6]
				...
				108^SCS^N = [09/22/2011^SCS^398,800^Sell^5.530000^Steelcase Inc.'s (NYSE:SCS) shares fell 18% a day after the office-furniture maker reported a second-quarter profit below analysts' consensus projection. ]*/
						...
						if ($arr_index_k[2] == 'P') {
							if ( ( ($arr_index_v[3] == 'Sell' || $arr_index_v[3] == 'Short') && $arr_index_v[5] < 0) || ( ($arr_index_v[3] == 'Buy' || $arr_index_v[3] == 'Cover') && $arr_index_v[5] > 0)) {
								$str_out_html .= '<tr class="ilt">
																			<td><font size="+1">'.$arr_index_k[1].'</font></td>
																			<td>Price</td>
																			<td>'.$arr_index_v[0].'</td>
																			<td><font size="+1">'.get_company_name($arr_index_k[1]).'</font></td>
																			<td>'.$arr_index_v[2].'</td>
																			<td>'.$arr_index_v[3].'</td>
																			<td>'.$arr_index_v[4].'</td>
																			<td>'.$arr_index_v[5].'%</td>
								...
								//include the file for holdings data.
								$v = format_date_mdy_to_ymd($arr_index_v[0]);
								$valsymbol = $arr_index_k[1];
								$str_out_html .= '<tr><td colspan="10">';
								$str_out_html .= get_holdings($v, $valsymbol);
								$str_out_html .= '</td>
																</tr>
																<tr>
																		<td colspan="10"><hr width="100%" size="2" noshade color="#0000FF" /></td>
																</tr>';
